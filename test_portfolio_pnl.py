...
def test_data_provider_methods():
    ...
    try:
        ...
        if activities:
            ...
            print(f"   Latest activity: {latest_activity.get('symbol')} {latest_activity.get('side')} {latest_activity.get('qty')}")
        ...
        if closed_pnl:
            for position in closed_pnl[:3]:  # Show first 3
                print(f"   {position['symbol']}: ${position['realized_pnl']:.2f} ({position['realized_pnl_pct']:.2f}%)")
