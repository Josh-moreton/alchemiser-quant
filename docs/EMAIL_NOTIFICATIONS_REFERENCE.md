...
This document catalogs all email notifications sent by the Octarine Capital trading system, including triggers, recipients, and content.
...
- **Note**: Environment is visible in the email subject line (e.g., `[DEV]`)
...
| `dev` | `true` | Any | `NOTIFICATIONS_TO_PROD` (dangerous) |
...
**Components**:
- `{Component}`: "Daily Run", "Data Lake Refresh", "Schedule", etc.
- `{STATUS}`: "SUCCESS", "SUCCESS_WITH_WARNINGS", "FAILURE", "RECOVERED", "SCHEDULED", etc.
- `{ENV}`: "DEV" or "STAGING" (only for non-production)
...
Daily Run - FAILURE
Daily Run - RECOVERED
Data Lake Refresh - SUCCESS
Schedule - EARLY CLOSE

# Non-production
[DEV] Daily Run - SUCCESS
[STAGING] Daily Run - FAILURE
[DEV] Data Lake Refresh - SUCCESS_WITH_WARNINGS
```

**Why This Format?**
- Production emails are clean and professional - no environment noise
- Non-production emails are clearly identified to prevent confusion
- Easy to filter in email clients (e.g., "Daily Run FAILURE")
- Quick identification of status at a glance
...
```json
{
  ...
  "extra": {
    ...
    "subject_preview": "Daily Run - SUCCESS",
...
- 1 daily run x 1 email/day = 30 emails/month
- Cost: **< $0.01/month**
...
**Status**: Current (SES migration complete)
