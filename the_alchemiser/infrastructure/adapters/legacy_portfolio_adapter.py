...
class LegacyPortfolioRebalancerAdapter:
    ...
    def calculate_rebalance_amounts(
        ...
        target_weights_decimal = {
            symbol: Decimal(str(weight)) for symbol, weight in target_weights.items()
        }
        current_values_decimal = {
            symbol: Decimal(str(value)) for symbol, value in current_values.items()
        }
    ...
    def get_portfolio_analysis(self) -> dict[str, Any]:
        ...
        if not self.use_new_system or self.portfolio_facade is None:
            ...
            return {
                ...
                "positions": [
                    {"symbol": pos.symbol, "value": pos.market_value, "qty": pos.qty}
    ...
    def get_symbols_needing_rebalance(self, target_weights: dict[str, float]) -> list[str]:
        ...
        return [symbol for symbol, plan in rebalance_plan.items() if plan.needs_rebalance]
    ...
    def _legacy_execute_rebalancing(
        self, target_weights: dict[str, float], dry_run: bool
    ) -> dict[str, Any]:
    ...
    def compare_systems(self, target_weights: dict[str, float]) -> dict[str, Any]:
        ...
        for symbol in all_symbols:
            ...
            if legacy_data and new_data:
                differences[symbol] = {
                    "trade_amount_diff": abs(
                        legacy_data.get("trade_amount", 0) - new_data.get("trade_amount", 0)
                    ),
                    "weight_diff_match": abs(
                        legacy_data.get("weight_diff", 0) - new_data.get("weight_diff", 0)
                    )
                    < 0.001,
                    "needs_rebalance_match": legacy_data.get("needs_rebalance")
                    == new_data.get("needs_rebalance"),
                }
