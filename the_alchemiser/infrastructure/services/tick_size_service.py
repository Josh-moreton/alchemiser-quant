...
class DynamicTickSizeService:
    ...
    def get_tick_size(self, symbol: str, current_price: Decimal | None = None) -> Decimal:
        ...
        if current_price is not None and current_price > Decimal("0"):
            ...
            self.logger.debug(f"Using price-based tick size {tick_size} for {symbol} at ${current_price}")
            return tick_size
...
_global_tick_size_service: DynamicTickSizeService | None = None


def get_tick_size_service() -> DynamicTickSizeService:
    """Get the global tick size service instance.
    
    Returns:
        Global DynamicTickSizeService instance

    """
    global _global_tick_size_service
    if _global_tick_size_service is None:
        _global_tick_size_service = DynamicTickSizeService()
    return _global_tick_size_service


def get_tick_size(symbol: str, current_price: Decimal | None = None) -> Decimal:
    """Get tick size for a symbol using the global service.
    
    Args:
        symbol: Trading symbol
        current_price: Optional current price for price-based rules
        
    Returns:
        Appropriate tick size

    """
    return get_tick_size_service().get_tick_size(symbol, current_price)