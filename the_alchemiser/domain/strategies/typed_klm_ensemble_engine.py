...
class TypedKLMStrategyEngine(StrategyEngine):
    ...
    def _calculate_indicators(self, market_data: dict[str, pd.DataFrame]) -> dict[str, dict[str, float]]:
    ...
    def _evaluate_ensemble(
        ...
        detailed_reason = self._build_detailed_klm_analysis(
            indicators, market_data, best_variant, symbol_or_allocation, 
            action, reason, variant_results
        )
        ...
        for variant in self.strategy_variants:
            try:
                ...
                try:
                    ...
                    if hasattr(result, '__getitem__') and hasattr(result, '__len__') and len(result) > 0:
    ...
    def _select_best_variant(
        ...
        self.logger.info(f"Selected variant: {best_variant.name} (performance: {best_performance:.4f})")
        self.logger.debug(
