...
from the_alchemiser.domain.dsl.ast import (
    ASTNode, NumberLiteral, Symbol, GreaterThan, LessThan, If,
...
class DSLEvaluator:
    ...
    def _evaluate_comparison(self, node: Union[GreaterThan, LessThan], operator: str) -> bool:
    ...
    def _evaluate_group(self, node: Group) -> Portfolio:
        ...
        if not isinstance(result, dict):
            raise PortfolioError(f"Group expression must evaluate to portfolio", operation="group")
    ...
    def _evaluate_weight_specified(self, node: WeightSpecified) -> Portfolio:
        ...
        for weight, expr in node.weights_and_expressions:
            result = self._evaluate_node(expr)
            if isinstance(result, dict):
                # Scale the portfolio by the weight
                scaled = {symbol: w * weight for symbol, w in result.items()}
    ...
    def _evaluate_weight_inverse_volatility(self, node: WeightInverseVolatility) -> Portfolio:
        ...
        self._trace_entries.append({
            ...
            "input_portfolios": len(portfolios),
            "weights": equal_weighted,
            "note": "Simplified equal weighting - TODO: implement inverse volatility"
        })
    ...
    def _evaluate_filter(self, node: Filter) -> Portfolio:
        ...
        # This is a placeholder - real implementation would apply the metric function
        # to each asset, rank them, and select the top N
        raise EvaluationError("Filter selector not yet implemented")
