...
def _stable_value(value: Any) -> tuple[Any, ...]:
    ...
    Recursively handles nested dataclasses, lists, dicts, and primitives
    to create a canonical representation for cache keys.
    """
    if is_dataclass(value):
        return ("NODE",) + node_key(value)
        ...
        # Primitives: int, float, str, bool, None, enums
        return ("PRIM", value)
...
def _canonicalise_recursive(node: ASTNode) -> ASTNode:
    ...
    canonicalised_fields = {}
    for field in fields(node):
        # Skip fields that are not part of the constructor (like node_id)
        if field.init:
            value = getattr(node, field.name)
    ...
    # Intern the node with canonicalised children
    return intern_node(fresh_node)


def _canonicalise_value(value: Any) -> Any:
    """Canonicalise a field value, handling various types."""
    if is_dataclass(value):
        return _canonicalise_recursive(value)
    elif isinstance(value, list):
