...
Examples:
  ...
  python -m the_alchemiser.backtest.cli preload --start 2023-01-01 --end 2024-01-01
        ...
        individual_parser = subparsers.add_parser(
            'individual', 
            help='Run individual strategy backtest'
        )
        individual_parser.add_argument(
            'strategy',
            choices=['nuclear', 'tecl', 'klm'],
        ...
        live_parser = subparsers.add_parser(
            'live',
            help='Run backtest using live trading configuration'
        )
        ...
        combinations_parser = subparsers.add_parser(
            'combinations',
            help='Run all weight combinations backtest'
        )
        combinations_parser.add_argument(
            '--step-size',
            type=int,
            default=10,
            help='Weight step size in percent (default: 10)'
        )
        combinations_parser.add_argument(
            '--max-workers',
            type=int,
        ...
        combinations_parser.add_argument(
            ...
            type=int,
            default=10,
            help='Show top N results (default: 10, 0 for all)'
        )
        ...
        preload_parser.add_argument(
            '--symbols',
            nargs='*',
            help='Specific symbols to cache (default: auto-detect all)'
        )
    ...
    def _add_common_args(self, parser: argparse.ArgumentParser):
        ...
        parser.add_argument(
            ...
            type=int,
            default=8,
            help='Slippage in basis points (default: 8)'
        )
        parser.add_argument(
            '--noise-factor',
            type=float,
        ...
        parser.add_argument(
            ...
            type=float,
            default=0.0,
            help='Regular deposit amount (default: 0)'
        )
        parser.add_argument(
            '--deposit-frequency',
            choices=['monthly', 'weekly'],
            help='Deposit frequency'
        )
        parser.add_argument(
            '--deposit-day',
            type=int,
            default=1,
            help='Day for deposits (default: 1)'
        )
        ...
        parser.add_argument(
            '--minute-data',
            action='store_true',
            help='Use minute data for execution'
        )
        ...
        if hasattr(args, 'start') and hasattr(args, 'end'):
            try:
                ...
                if (end_date - start_date).days < 30:
                    console.print("[yellow]⚠️ Warning: Very short backtest period (< 30 days)[/yellow]")
                
                if (end_date - start_date).days > 365 * 10:
                    console.print("[yellow]⚠️ Warning: Very long backtest period (> 10 years)[/yellow]")
    ...
    def display_results_table(self, results: List, title: str = "Backtest Results"):
        ...
        for i, result in enumerate(results, 1):
            ...
            if hasattr(result, 'strategy_name'):
                ...
                calmar_str = f"{result.calmar_ratio:.2f}" if result.calmar_ratio != float('inf') else "∞"
                table.add_row(
                    str(i),
