...
from the_alchemiser.backtest.data_cache import BacktestDataCache, get_global_cache
...
class DataLoader:
    ...
    def get_data_provider(self) -> UnifiedDataProvider:
        ...
        if self.data_provider is None:
            self.data_provider = UnifiedDataProvider(paper_trading=True, cache_duration=3600)
    ...
    def calculate_optimized_lookback(
        ...
        if use_minute_candles:
            ...
            console.print("[yellow]🕐 Using minute data - shorter lookback recommended[/yellow]")
        ...
        console.print(
            f"[green]📊 Optimized lookback: {total_lookback} days (vs {DEFAULT_LOOKBACK_DAYS} default)[/green]"
        )
        console.print(
            f"[dim]   Max symbol requirement: {max_lookback} days + {safety_buffer} day buffer[/dim]"
        )
        ...
        try:
            ...
            console.print(
                f"[dim]   Data reduction: {savings_pct:.1f}% ({old_approach_days:,} → {new_approach_days:,} symbol-days)[/dim]"
            )
        except Exception as e:
    ...
    def _load_data_direct(
        ...
        Returns:
            Tuple of (daily_data, minute_data) dictionaries
        """
        console.print("[yellow]⚠️ Loading data directly without cache[/yellow]")
        ...
        for i, symbol in enumerate(symbols):
            try:
                ...
                bars = data_provider.get_historical_data(
                ...
                if bars:
                    ...
                    for bar in bars:
                        try:
                            ...
                            console.print(
                                f"[red]❌ Error processing bar for {symbol}: {bar_error}[/red]"
                            )
                            continue
    ...
    def get_cached_symbol_data(
        ...
        if missing_symbols:
            console.print(
                f"[yellow]⚠️ Missing from cache: {missing_symbols[:5]}{'...' if len(missing_symbols) > 5 else ''}[/yellow]"
            )
