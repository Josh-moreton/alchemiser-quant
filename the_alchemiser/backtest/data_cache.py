...
class BacktestDataCache:
    ...
    def get_cache_key(self, start_date: dt.datetime, end_date: dt.datetime, 
                     symbols: List[str], include_minute_data: bool = False) -> str:
    ...
    def save_cache_to_disk(self, cache_key: str, daily_data: Dict[str, pd.DataFrame], 
                          minute_data: Optional[Dict[str, pd.DataFrame]] = None) -> bool:
    ...
    def load_cache_from_disk(self, cache_key: str) -> Tuple[Dict[str, pd.DataFrame], Dict[str, pd.DataFrame]]:
    ...
    def get_all_required_symbols(self, include_benchmarks: bool = True) -> Set[str]:
        ...
        if include_benchmarks:
            benchmark_symbols = {
                ...
                'VTI', 'VEA', 'VWO',         # Broad market ETFs
                'TLT', 'SHY', 'GLD', 'SLV',  # Alternative assets
    ...
    def preload_all_data(self, start_date: dt.datetime, end_date: dt.datetime, 
            ...
            try:
                ...
                if bars:
                        ...
                        try:
                            if hasattr(bar, 'open') and hasattr(bar, 'high'):  # Validate bar structure
                                ...
                                if timestamp is None:
                                    console.print(f"[red]⚠️ No timestamp found for {symbol} bar {i}[/red]")
                                    continue
                                ...
                                try:
                                    ...
                                    console.print(f"[red]⚠️ Error processing timestamp for {symbol} bar {i}: {ts_error}[/red]")
                                    continue
                    ...
                    if data_rows and timestamps:
                        ...
                        console.print(f"[red]⚠️ No valid bars for {symbol} (got {len(data_rows)} rows, {len(timestamps)} timestamps)[/red]")
                else:
                    failed_symbols.append(symbol)
...
def preload_backtest_data(start_date: dt.datetime, end_date: dt.datetime,
                         symbols: Optional[List[str]] = None,
                         include_minute_data: bool = False,
                         force_refresh: bool = False) -> Tuple[Dict[str, pd.DataFrame], Dict[str, pd.DataFrame]]:
    """
