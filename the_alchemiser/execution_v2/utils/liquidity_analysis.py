...
class LiquidityAnalyzer:
    ...
    def validate_liquidity_for_order(
        ...
        if available_volume < self.min_volume_threshold:
            return False, f"Insufficient volume at {price_level}: {available_volume} < {self.min_volume_threshold}"
            
        # Check volume ratio
        volume_ratio = order_size / available_volume
        if volume_ratio > 2.0:  # Order more than 2x available volume
            return False, f"Order too large vs liquidity: {order_size} vs {available_volume} at {price_level}"
