...
class RepegManager:
            ...
            if getattr(executed_order, "success", False) and getattr(
                ...
                if self._is_valid_uuid_str(new_order_id):
                    self.order_tracker.update_order(
                        order_id, new_order_id, new_price, datetime.now(UTC)
                    )
    ...
    async def _place_limit_with_insufficient_retry(
        ...
        try:
            ...
            Exception
        ) as e:  # Specific SDK error types can be handled here if exposed
            error_str = str(e)
            if "insufficient qty available" in error_str.lower():
                ...
                if available_match:
                    try:
                        ...
                        logger.error(
                            f"❌ Retry with available quantity failed: {retry_e}"
                        )
                        raise e
            raise e
