...
class RepegManager:
    ...
    async def _attempt_repeg(
        ...
        try:
            ...
            replace_result = await self._replace_for_repeg(
                order_id, quantized_price, remaining_qty
            )
            ...
            if replace_result and replace_result.success:
                ...
                if quote is None:
                    raise ValueError(
                        "Quote should be validated as non-None when new_price exists"
                    )
                q = quote
            ...
            # Fallback: if replace failed, try cancel+resend to avoid stalling the order
            logger.warning(
                f"⚠️ Re-peg replacement failed for {request.symbol}. Falling back to cancel+resend."
            )
            cancel_ok = await self._cancel_for_repeg(order_id)
            if not cancel_ok:
                # Could not cancel; report failure but keep tracking so we may retry later
                return SmartOrderResult(
                    success=False,
                    error_message="Replace failed and cancel could not be confirmed",
                    execution_strategy="replace_then_cancel_failed",
                    repegs_used=new_repeg_count,
                )

            # Place a fresh limit order with the same improved price and remaining qty
            try:
                placed = await self._place_limit_with_insufficient_retry(
                    request, remaining_qty, quantized_price
                )
                if placed and placed.success:
                    new_id = placed.order_id
                    # Update tracking to the new order id
                    self.order_tracker.update_order(
                        order_id, new_id, quantized_price, datetime.now(UTC)
                    )
                    logger.info(
                        f"✅ Re-peg successful via cancel+resend: new order {new_id} at ${quantized_price} "
                        f"(attempt {new_repeg_count}/{self.config.max_repegs_per_order}) quantity: {remaining_qty}"
                    )
                    ...
                    return SmartOrderResult(
                        ...
                        anchor_price=original_anchor,
                        repegs_used=new_repeg_count,
                        execution_strategy=f"cancel_resend_repeg_{new_repeg_count}",
                        placement_timestamp=datetime.now(UTC),
                        metadata=metadata_dict,
                    )

                logger.error(
                    f"❌ Cancel+resend after replace failure also failed for {request.symbol}: status={placed.status if placed else 'unknown'}"
                )
                return SmartOrderResult(
                    success=False,
                    error_message="Replace failed; cancel+resend placement failed",
                    execution_strategy="replace_then_cancel_resend_failed",
                    repegs_used=new_repeg_count,
                )
            except Exception as e:
                logger.error(
                    f"❌ Error during cancel+resend fallback for {request.symbol}: {e}"
                )
                return SmartOrderResult(
                    success=False,
                    error_message=str(e),
                    execution_strategy="replace_then_cancel_resend_error",
                    repegs_used=new_repeg_count,
                )
    ...
    async def _replace_for_repeg(
        ...
        try:
            logger.info(
                f"🔄 Replacing order {order_id} with new price ${new_price} and quantity {remaining_qty}"
            )
            ...
            if result.success:
                logger.info(
                    f"✅ Successfully replaced order {order_id} -> {result.order_id} at ${new_price}"
                )
                return SmartOrderResult(
                    success=True,
            ...
            error_msg = (
                getattr(result, "error", "Unknown error") or "Order replacement failed"
            )
