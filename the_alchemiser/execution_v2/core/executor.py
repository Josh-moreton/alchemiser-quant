...
from the_alchemiser.shared.schemas.execution.execution_result import ExecutionResult
...
class Executor:
        ...
        *,
        ...
        if enable_smart_execution:
            try:
                ...
                logger.info(
                    "✅ Smart execution strategy initialized with shared WebSocket"
                )
    ...
    def _execute_market_order(
        ...
        if not validation_result.is_valid:
            error_msg = (
                validation_result.error_message or f"Validation failed for {symbol}"
            )
        ...
        try:
            ...
            if side.lower() == "buy":
                ...
                if not is_sufficient and estimated_cost:
                    ...
                    if self.buying_power_service.force_account_refresh():
                        logger.info(
                            "💰 Account refreshed, proceeding with order attempt"
                        )
                    else:
            ...
            if "insufficient buying power" in error_str.lower():
                ...
                try:
                    ...
                    logger.debug(
                        f"Diagnostic account retrieval failed: {diagnostic_error}"
                    )
    ...
    async def _execute_sell_phase(
        self, sell_items: list[RebalancePlanItem]
    ...
    async def _execute_buy_phase(
        self, buy_items: list[RebalancePlanItem]
    ) -> tuple[list[OrderResult], ExecutionStats]:
    ...
    def _log_monitoring_completion(
        self, phase_type: str, start_time: float, attempts: int
    ) -> None:
    ...
    def _log_repeg_status(
        self, phase_type: str, repeg_result: Any
    ) -> None:  # noqa: ANN401
