...
class Executor:
    ...
    async def execute_order(
        ...
        # If extended-hours is enabled at the broker, use a DAY LIMIT fallback at inside price
        try:
            if getattr(self.alpaca_manager, "extended_hours_enabled", False):
                return self._execute_extended_hours_limit_fallback(
                    symbol, side, Decimal(str(quantity))
                )
        except Exception as _eh_exc:
            logger.debug(
                f"Extended-hours fallback check failed, using market fallback: {_eh_exc}"
            )

        logger.info(f"📈 Using standard market order for {symbol}")
        return self._execute_market_order(symbol, side, Decimal(str(quantity)))

    def _execute_extended_hours_limit_fallback(
        self, symbol: str, side: str, quantity: Decimal
    ) -> OrderResult:
        """Execute a DAY LIMIT fallback at inside price for extended-hours compliance.
        ...
        Notes:
            ...
            - Uses TIF=DAY as required by broker for extended hours.
            - Falls back to market order only if no pricing info is available.

        """
        # Preflight validation (handles non-fractionable rounding)
        ...
        # Extended-hours safeguard: avoid fractional orders which may be rejected
        final_quantity = self._round_quantity_for_extended_hours(final_quantity)

        try:
            # Determine inside price for fallback
            limit_price, side_text = self._get_inside_price_for_extended_hours(
                symbol, side
            )

            if limit_price is None or limit_price <= 0:
                logger.warning(
                    f"⚠️ No quote available for extended-hours fallback for {symbol}; using market fallback"
                )
                return self._execute_market_order(symbol, side, final_quantity)

            # Quantize to cents
            limit_price = Decimal(str(float(limit_price))).quantize(Decimal("0.01"))

            logger.info(
                f"📈 Using extended-hours-safe LIMIT fallback for {symbol}: "
                f"{side.upper()} at {side_text} ${limit_price} (TIF=DAY, extended_hours=True)"
            )

            # Place DAY LIMIT order
            broker_result = self.alpaca_manager.place_limit_order(
                symbol=symbol,
                side=side.lower(),
            ...
            success = getattr(broker_result, "status", "accepted") not in [
                "rejected",
                "canceled",
            ]
            error_msg = (
                None
                if success
                else getattr(broker_result, "error", None) or "Order rejected"
            )

            return OrderResult(
                symbol=symbol,
                action=side.upper(),
                trade_amount=abs(final_quantity * limit_price),
            ...
            logger.error(f"❌ Extended-hours LIMIT fallback failed for {symbol}: {exc}")
            return OrderResult(
                symbol=symbol,
                action=side.upper(),
    ...
    def _get_inside_price_for_extended_hours(
        self, symbol: str, side: str
    ) -> tuple[Decimal | None, str]:
        """Get an appropriate inside price and label for extended-hours fallback.

        Returns:
            (limit_price, side_text), where side_text is one of {"ASK", "BID", "MID"}.
        ...
        try:
            ...
            logger.debug(
                f"Quote lookup failed for {symbol} during EH fallback: {q_exc}"
            )
        ...
        if (
            ...
            if side.upper() == "BUY":
                return Decimal(str(quote.ask)), "ASK"
            return Decimal(str(quote.bid)), "BID"

        est_price = self._get_price_for_estimation(symbol)
        if est_price is not None and est_price > Decimal("0"):
            return est_price, "MID"

        return None, "MID"

    def _round_quantity_for_extended_hours(self, quantity: Decimal) -> Decimal:
        """Round quantity down to whole shares in extended hours if fractional.

        This is a safety measure to avoid broker rejections for some assets that
        disallow fractional trading during extended hours.
        """
        try:
            if quantity != quantity.to_integral_value():
                adjusted = quantity.quantize(Decimal("1"), rounding=ROUND_DOWN)
                if adjusted <= 0:
                    return Decimal("0")
                logger.info(
                    f"🔄 Extended-hours safeguard: adjusted fractional quantity {quantity} → {adjusted} shares"
                )
                return adjusted
        except Exception:
            # If anything goes wrong, return original quantity
            return quantity
        return quantity
    ...
    def _execute_market_order(
        ...
        try:
            if side.lower() == "buy":
                # Enforce buying power sufficiency before placing BUY market orders
                (
                    is_sufficient,
                    current_bp,
                    estimated_cost,
                ...
                if not is_sufficient:
                    logger.error(
                        f"💳 Blocking BUY {symbol} market order due to insufficient buying power: "
                        f"have ${current_bp}, need ~${estimated_cost}"
                    )
                    return OrderResult(
                        symbol=symbol,
                        action=side.upper(),
                        ...
                        success=False,
                        error_message=(
                            f"Insufficient buying power (have ${current_bp}, need ~${estimated_cost})"
                            if estimated_cost is not None
                            else "Insufficient buying power"
                        ),
                        timestamp=datetime.now(UTC),
                    )
    ...
    def _build_market_order_execution_result(
        ...
        # Surface broker-provided error details (e.g., 422 not tradable, halts)
        error_msg: str | None = None
        if not success:
            # Prefer explicit broker error message when available; fallback to status text
            error_msg = (
                broker_result.error_message
                if getattr(broker_result, "error_message", None)
                else f"Order {broker_result.status}"
            )
