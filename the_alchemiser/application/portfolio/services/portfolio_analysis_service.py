...
from the_alchemiser.application.trading.trading_service_manager import (
...
class PortfolioAnalysisService:
    ...
    def get_comprehensive_portfolio_analysis(self) -> dict[str, Any]:
        ...
        strategy_allocations = self.attribution_engine.calculate_strategy_allocations(
            positions, portfolio_value
        )
        ...
        return {
            ...
            "risk_metrics": {
                "concentration_risk": concentration_metrics.get(
                    "top_10_concentration", Decimal("0")
                ),
    ...
    def analyze_portfolio_drift(self, target_weights: dict[str, Decimal]) -> dict[str, Any]:
        ...
        current_weights = {
            symbol: value / portfolio_value for symbol, value in current_positions.items()
        }
        ...
        return {
            ...
            "overall_metrics": {
                ...
                "average_drift": (
                    total_absolute_drift / len(drift_analysis) if drift_analysis else Decimal("0")
                ),
                "symbols_needing_rebalance": sum(
                    1 for analysis in drift_analysis.values() if analysis["requires_rebalancing"]
                ),
        ...
        for strategy, strategy_positions in strategy_groups.items():
            ...
            strategy_analysis[strategy] = {
                ...
                "average_position_size": (
                    strategy_value / len(strategy_positions) if strategy_positions else Decimal("0")
                ),
                "strategy_description": self.attribution_engine.classifier.get_strategy_description(
                    strategy
                ),
            }
    ...
    def compare_target_vs_current_strategy_allocation(
        ...
        # Compare allocations
        comparison = {}
        all_strategies = set(current_strategy_allocations.keys()) | set(
            target_strategy_allocations.keys()
        )
    ...
    def _calculate_concentration_metrics(
        ...
        top_1 = (
            sorted_positions[0] / portfolio_value if len(sorted_positions) >= 1 else Decimal("0")
        )
        top_5 = (
            sum(sorted_positions[:5]) / portfolio_value
            if len(sorted_positions) >= 5
            else sum(sorted_positions) / portfolio_value
        )
        top_10 = (
            sum(sorted_positions[:10]) / portfolio_value
    ...
    def _create_strategy_summary(self, strategy_exposures: dict[str, Any]) -> dict[str, Any]:
        ...
        largest_strategy = max(
            strategy_exposures.items(), key=lambda x: x[1]["allocation_percentage"]
        )
