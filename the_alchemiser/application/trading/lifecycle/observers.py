...
class LoggingObserver:
    ...
    def on_lifecycle_event(self, event: OrderLifecycleEvent) -> None:
        ...
        if event.is_error_transition():
            ...
            log_message = f"Order {event.order_id} completed: {event.previous_state} -> {event.new_state}"
        else:
            log_level = logging.DEBUG
            log_message = f"Order {event.order_id} transition: {event.previous_state} -> {event.new_state}"
