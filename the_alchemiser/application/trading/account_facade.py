...
class AccountFacade:
    ...
    def get_account_info(self) -> AccountInfo:
        ...
        except (DataProviderError, TradingClientError, ConnectionError, TimeoutError, AttributeError) as e:
    ...
    def get_enriched_account_info(self, paper_trading: bool = True) -> EnrichedAccountInfo:
        ...
        try:
            ...
            # Note: Portfolio history typically requires direct AlpacaManager access
            # since it's not exposed through standard AccountService interface
            try:
                # Check if account service has access to portfolio history
                if hasattr(self._account_service.account_repository, 'get_portfolio_history'):
                    portfolio_history = self._account_service.account_repository.get_portfolio_history()
                ...
                self.logger.debug(f"Could not retrieve portfolio history: {e}")

            # Add recent closed P&L if available
            # Note: This would need implementation in the underlying repository
            try:
                # Check if account service supports closed positions
                if hasattr(self._account_service.account_repository, 'get_recent_closed_positions'):
                    closed_pnl = self._account_service.account_repository.get_recent_closed_positions()
                    if closed_pnl:
                ...
                self.logger.debug(f"Could not retrieve recent closed P&L: {e}")
    ...
    def get_current_price(self, symbol: str) -> float:
        ...
        try:
            ...
            if self._market_data_service is not None:
                ...
                if price is not None and price > 0:
                    self.logger.debug(f"Retrieved price from MarketDataService for {symbol}: ${price:.2f}")
                    return price
        ...
        except (DataProviderError, ConnectionError, TimeoutError, ValueError, AttributeError, NotImplementedError) as e:
            ...
            if self._market_data_service is not None:
                try:
                    ...
                    if hasattr(self._market_data_service, 'get_current_prices'):
                        prices = getattr(self._market_data_service, 'get_current_prices')(valid_symbols)
                        if prices:
                            # Validate prices
                            valid_prices = {}
    ...
    def get_enriched_account_summary(self) -> EnrichedAccountSummaryDTO:
        ...
        - Returning both raw and typed representations
        
        Returns:
            EnrichedAccountSummaryDTO with both raw and typed representations
        """
        try:
