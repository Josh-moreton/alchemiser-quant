    ...
    def validate_and_adjust(
        ...
        if not is_fractionable:
            ...
            if was_adjusted:
                ...
                log_with_context(
                    logger,
                    logging.INFO,
                    f"Adjusted quantity for non-fractionable asset",
                    policy=self.policy_name,
                    action="adjust",
                    symbol=order_request.symbol,
                    original_quantity=str(original_quantity),
        ...
        if adjusted_quantity <= 0:
            log_with_context(
                logger,
                logging.WARNING,
                f"Order rejected: final quantity is zero or negative",
                policy=self.policy_name,
                action="reject",
                symbol=order_request.symbol,
                adjusted_quantity=str(adjusted_quantity),
        ...
        log_with_context(
            logger,
            logging.INFO,
            f"Fractionability validation passed",
            policy=self.policy_name,
            action="allow",
            symbol=order_request.symbol,
