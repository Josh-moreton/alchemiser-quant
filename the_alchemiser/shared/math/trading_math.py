...
def _log_enhanced_threshold_analysis(
    ...
    if data.current_value > 0 and data.target_weight > 0:
        ...
        logger.info(
            f"IMPLIED_PORTFOLIO_VALUE_FROM_{data.symbol}: ${implied_portfolio_value:.2f}"
        )
...
def _log_critical_bug_detection(
    ...
    if (
        data.target_weight > 0.01
        and abs(data.weight_diff) > 0.05
        and not data.needs_rebalance
    ):
        logger.error(
    ...
    if (
        math.isclose(data.trade_amount, 0.0, abs_tol=1e-10)
        and data.target_weight > 0.01
    ):
        logger.error(
    ...
    if (
        math.isclose(data.total_portfolio_value, 0.0, abs_tol=1e-10)
        and data.target_weight > 0
    ):
        logger.error(
