...
class RealTimePricingService:
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            self._stream = create_stock_data_stream(
                ...
                feed=self._get_feed(),
                websocket_params=self._build_websocket_params(),
            )
    ...
    async def _handle_stream_error(
        ...
        if not self._should_reconnect and isinstance(error, timeout_types):
            logging.debug(
                "Websocket connection timed out during intentional shutdown; skipping retry"
            )
            return False
    ...
    async def _on_quote(self, data: AlpacaQuoteData) -> None:
        ...
        """
        try:
            symbol = self._extract_symbol_from_quote(data)
            if not symbol:
                self.logger.warning("Received quote with no symbol")
                return

            quote_values = self._extract_quote_values(data)
            timestamp = self._get_quote_timestamp(quote_values.timestamp_raw)

            await self._process_quote_data(symbol, quote_values, timestamp, data)
            self._update_quote_statistics(symbol)

        except Exception as e:
            await self._handle_quote_error(e)

    def _extract_symbol_from_quote(self, data: AlpacaQuoteData) -> str:
        """Extract symbol from quote data."""
        if hasattr(data, "symbol"):
            return str(data.symbol)
        return str(data.get("S", "")) if isinstance(data, dict) else ""

    def _extract_quote_values(self, data: AlpacaQuoteData) -> QuoteValues:
    ...
    async def _update_bid_ask_data(
        self, symbol: str, quote_values: QuoteValues, timestamp: datetime
    ) -> None:
    ...
    def _build_websocket_params(self) -> dict[str, float]:
        """Construct websocket parameter overrides with environment support."""

        def _read_timeout(name: str, default: float, minimum: float) -> float:
            ...
            try:
                ...
                self.logger.warning(
                    "Invalid %s override '%s'; using default %.1f",
                    name,
                    raw_value,
                    default,
                )
            ...
            if parsed < minimum:
                self.logger.warning(
                    "%s override %.1f below minimum %.1f; using minimum",
                    name,
                    parsed,
                    minimum,
                )
        ...
        return {
            "open_timeout": _read_timeout("ALPACA_WS_OPEN_TIMEOUT", 30.0, 5.0),
            "close_timeout": _read_timeout("ALPACA_WS_CLOSE_TIMEOUT", 10.0, 1.0),
            "ping_interval": _read_timeout("ALPACA_WS_PING_INTERVAL", 20.0, 5.0),
            "ping_timeout": _read_timeout("ALPACA_WS_PING_TIMEOUT", 20.0, 5.0),
        }
    ...
    def bulk_subscribe_symbols(
        ...
        """Subscribe to multiple symbols in a single operation to minimize restarts.

        Args:
            symbols: List of stock symbols to subscribe to
            priority: Priority score for all symbols (higher = more important)

        Returns:
            Dictionary mapping symbol to subscription success (True/False)
        ...
        with self._subscription_lock:
            ...
            symbols_to_add = self._update_existing_symbols_priority(
                symbols, priority, results
            )
            ...
            symbols_to_replace = self._find_symbols_to_replace_bulk(
                symbols_to_add, priority
            )
            ...
            if needs_restart:
                logging.info(
                    f"📡 Bulk subscribing to {sum(results.values())} symbols with priority {priority:.1f}"
                )
    ...
    def _restart_stream_for_new_subscription(self) -> None:
        """Restart the stream to pick up new subscriptions with proper cleanup.
        ...
        try:
            ...
            if self._stream:
                try:
                    self._stream.stop()
                    # CRITICAL: Add explicit cleanup and wait for connection to close
                    time.sleep(0.5)  # Give Alpaca time to close the connection
                    self._stream = None  # Clear the reference
                except Exception as e:
                    logging.debug(f"Error stopping stream for restart: {e}")

            # Wait for the stream thread to finish with longer timeout
            if self._stream_thread and self._stream_thread.is_alive():
                self._stream_thread.join(timeout=5.0)  # Increased from 2.0
                if self._stream_thread.is_alive():
                    logging.warning("⚠️ Stream thread didn't terminate cleanly")
                    return  # Don't restart if old thread is still running

            # Add backoff delay before reconnecting to prevent rapid attempts
            time.sleep(1.0)  # Prevent rapid reconnection attempts
            ...
            while time.time() - start_time < 10.0:  # Increased timeout
                ...
                logging.warning("⚠️ Stream restart timed out - may not be connected")
