...
class CrossModuleCommunicationHelper:
    ...
    def validate_dto_communication_chain(
        ...
        try:
            ...
            if not all(signal.correlation_id and signal.causation_id for signal in strategy_signals):
                return False
            ...
            timestamps = [
                min(signal.timestamp for signal in strategy_signals) if strategy_signals else datetime.now(UTC),
                portfolio_state.timestamp,
                min(order.timestamp for order in order_requests) if order_requests else datetime.now(UTC),
...
def example_strategy_to_portfolio_communication() -> dict[str, Any]:
    ...
    portfolio_context = {
        ...
        'allocation_weight': float(signal_dto.allocation_weight) if signal_dto.allocation_weight else 0.0,
    }
