...
def position_to_dto(
    ...
    quantity = get_attr_value(position, 'quantity') or get_attr_value(position, 'qty') or get_attr_value(position, 'Quantity')
    
    # Average cost / entry price
    average_cost = (
        get_attr_value(position, 'average_cost') or
        get_attr_value(position, 'avg_cost') or
        get_attr_value(position, 'average_entry_price') or
        get_attr_value(position, 'avg_entry_price') or
        get_attr_value(position, 'entry_price') or
        Decimal('0')
    )
    
    # Current price
    current_price = (
        get_attr_value(position, 'current_price') or
        get_attr_value(position, 'price') or
        get_attr_value(position, 'market_price') or
        average_cost  # Fallback to average cost if no current price
    )
    ...
    # Unrealized P&L
    unrealized_pnl = get_attr_value(position, 'unrealized_pnl') or get_attr_value(position, 'unrealized_pl')
    ...
    unrealized_pnl_percent = get_attr_value(position, 'unrealized_pnl_percent') or get_attr_value(position, 'unrealized_plpc')
    if unrealized_pnl_percent is None and average_cost and float(average_cost) != 0:
        ...
        unrealized_pnl_percent = (Decimal(str(current_price)) - Decimal(str(average_cost))) / Decimal(str(average_cost))
    ...
    return PositionDTO(
        ...
        unrealized_pnl_percent=Decimal(str(unrealized_pnl_percent)) if unrealized_pnl_percent is not None else Decimal('0'),
...
def portfolio_state_to_dto(
    ...
    total_value = get_value(portfolio_data, 'total_value') or get_value(portfolio_data, 'portfolio_value')
    ...
    buying_power = get_value(portfolio_data, 'buying_power') or get_value(portfolio_data, 'daytrading_buying_power')
    
    # P&L metrics
    day_pnl = get_value(portfolio_data, 'day_pnl') or get_value(portfolio_data, 'unrealized_pl')
    day_pnl_percent = get_value(portfolio_data, 'day_pnl_percent') or get_value(portfolio_data, 'unrealized_plpc')
    ...
    metrics = PortfolioMetricsDTO(
        ...
        day_pnl_percent=Decimal(str(day_pnl_percent)) if day_pnl_percent is not None else Decimal('0'),
        total_pnl=Decimal(str(total_pnl)) if total_pnl is not None else Decimal('0'),
        total_pnl_percent=Decimal(str(total_pnl_percent)) if total_pnl_percent is not None else Decimal('0'),
    ...
    if isinstance(strategy_allocations, dict):
        ...
        strategy_allocations = {
            k: Decimal(str(v)) for k, v in strategy_allocations.items()
        }
    else:
        strategy_allocations = {}
    ...
    return PortfolioStateDTO(
        ...
        max_position_size=Decimal(str(max_position_size)) if max_position_size is not None else None,
        rebalance_threshold=Decimal(str(rebalance_threshold)) if rebalance_threshold is not None else None,
...
def dto_to_portfolio_context(dto: PortfolioStateDTO) -> dict[str, Any]:
    ...
    return {
        ...
        'strategy_allocations': {
            k: float(v) for k, v in dto.strategy_allocations.items()
        },
