...
def order_result_to_executed_order_dto(
    ...
    order_id = (
        get_attr_value(order_result, "order_id") or get_attr_value(order_result, "id") or "unknown"
    )
    symbol = (
        get_attr_value(order_result, "symbol")
        or get_attr_value(order_result, "Symbol")
        or "UNKNOWN"
    )
    ...
    # Quantities
    quantity = (
        get_attr_value(order_result, "quantity")
        or get_attr_value(order_result, "qty")
        or Decimal("0")
    )
    filled_quantity = (
        get_attr_value(order_result, "filled_quantity")
        or get_attr_value(order_result, "filled_qty")
        or get_attr_value(order_result, "qty_filled")
        or quantity  # Assume fully filled if no filled quantity provided
    )

    # Price
    price = (
        get_attr_value(order_result, "price")
        or get_attr_value(order_result, "filled_price")
        or get_attr_value(order_result, "avg_fill_price")
        or get_attr_value(order_result, "limit_price")
        or Decimal("0")
    )
    ...
    # Status
    status = (
        get_attr_value(order_result, "status") or get_attr_value(order_result, "state") or "FILLED"
    )
    ...
    # Timestamp
    execution_timestamp = (
        get_attr_value(order_result, "execution_timestamp")
        or get_attr_value(order_result, "filled_at")
        or get_attr_value(order_result, "updated_at")
        or datetime.now(UTC)
    )
    ...
    error_message = get_attr_value(order_result, "error_message") or get_attr_value(
        order_result, "reject_reason"
    )
...
def create_execution_report_dto(
    ...
    execution_start_time = (
        min(o.execution_timestamp for o in executed_orders)
        if executed_orders
        else datetime.now(UTC)
    )
    execution_end_time = (
        max(o.execution_timestamp for o in executed_orders)
        if executed_orders
        else datetime.now(UTC)
    )
    ...
    success_rate = (
        Decimal(str(successful_orders / total_orders)) if total_orders > 0 else Decimal("0")
    )
...
def order_request_to_context(dto: OrderRequestDTO) -> dict[str, Any]:
    ...
    return {
        ...
        "max_slippage_percent": float(dto.max_slippage_percent)
        if dto.max_slippage_percent
        else None,
