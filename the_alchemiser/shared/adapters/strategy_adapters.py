...
def validate_signal_conversion(
    ...
    try:
        # Check symbol match
        original_symbol = str(original_signal.symbol) if hasattr(original_signal.symbol, '__str__') else original_signal.symbol
        ...
        if hasattr(original_signal, 'reasoning') and original_signal.reasoning != converted_dto.reasoning:
            return False
