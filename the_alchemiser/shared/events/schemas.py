...
class SignalGenerated(BaseEvent):
    ...
    signals: list[StrategySignalDTO] = Field(
        ..., description="List of generated strategy signals"
    )
...
class RebalancePlanned(BaseEvent):
    ...
    rebalance_plan: RebalancePlanDTO = Field(
        ..., description="Portfolio rebalancing plan"
    )
    portfolio_state: PortfolioStateDTO = Field(
