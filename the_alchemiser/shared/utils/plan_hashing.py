...
def generate_plan_hash(
    ...
    try:
        ...
        logger.debug(f"Generated plan hash: {plan_hash} from {len(json_data)} bytes")
        return plan_hash
...
def _normalize_plan_for_hash(plan: RebalancePlanDTO) -> dict[str, Any]:
    ...
    stable_plan = {key: value for key, value in plan_dict.items() if key not in volatile_fields}
    ...
    if "items" in stable_plan and isinstance(stable_plan["items"], list):
        stable_plan["items"] = sorted(
            stable_plan["items"], key=lambda x: x.get("symbol", "") if isinstance(x, dict) else ""
        )
    ...
    for field in allocation_fields:
        if field in stable_comparison and isinstance(stable_comparison[field], dict):
            ...
            stable_comparison[field] = {
                symbol: str(value) for symbol, value in sorted(stable_comparison[field].items())
            }
