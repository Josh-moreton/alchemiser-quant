...
class TradingSystem:
    ...
    def _display_signals_summary(self, signals_result: dict[str, Any]) -> None:
        ...
        try:
            ...
            if isinstance(strategy_signals, dict) and strategy_signals:
                # Display each strategy's signal separately
                        ...
                        if action in {"BUY", "SELL", "HOLD"}:
                            if data.get("is_multi_symbol") and isinstance(
                                ...
                                if symbols:
                                    ...
                                    print(f"🎯 {name} strategy → {action} {symbol_str}")
                            elif data.get("symbol"):
                                print(
                                    f"🎯 {name} strategy → {action} {data.get('symbol')}"
                                )
                            else:
                                print(f"🎯 {name} strategy → {action}")
            else:
            ...
            if "consolidated_portfolio" in signals_result:
                ...
                if isinstance(portfolio, dict):
                    ...
                    if non_zero:
                        ...
                        print(f"📊 Final consolidated allocations: {allocations}")
                    else:
                        print("📊 Final consolidated allocations: 100% cash")
        except Exception as e:
