        ...
        if hasattr(result, 'iloc') and len(result) > 0:
            ...
            if pd.isna(value):
                ...
                if len(valid_values) > 0:
                    ...
                    logging.error(f"No valid values for indicator {indicator_func.__name__} on data: {data.tail(1)}")
                    return 50.0  # Fallback only if no valid values
            return float(value)
        logging.error(f"Indicator {indicator_func.__name__} returned no results for data: {data.tail(1)}")
        return 50.0
