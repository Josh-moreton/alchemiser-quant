...
class SymbolLookbackCalculator:
    ...
    def get_symbol_lookback_days(self, symbol: str, strategies: Optional[List[str]] = None) -> int:
        ...
        self.logger.debug(f"Symbol {symbol}: {max_lookback} days lookback "
                         f"(indicators: {', '.join(sorted(used_indicators))})")
        
        return max_lookback
    
    def get_all_symbols_lookback_map(self, strategies: Optional[List[str]] = None) -> Dict[str, int]:
        """
        ...
        self.logger.info(f"Generated lookback map for {len(lookback_map)} symbols "
    ...
    def _get_strategy_indicators(self, strategy: str) -> Dict[str, IndicatorRequirement]:
        ...
        return {name: self._indicator_requirements[name] 
    ...
    def optimize_data_fetching(self, symbols: List[str], strategies: Optional[List[str]] = None) -> Dict[str, Dict[str, Any]]:
        """
        ...
        total_days_optimized = sum(days * len(symbols_list) 
                                 for days, symbols_list in lookback_groups.items())
        
        efficiency_gain = ((total_days_naive - total_days_optimized) / total_days_naive * 100 
                          if total_days_naive > 0 else 0)
        ...
        return {
            ...
            'efficiency_metrics': {
                ...
                'data_reduction_ratio': total_days_optimized / total_days_naive if total_days_naive > 0 else 1
            }
        }
...
def get_optimized_lookback_map(symbols: List[str], strategies: Optional[List[str]] = None) -> Dict[str, int]:
    """
...
def main():
    ...
    for strategy in ['nuclear', 'tecl', 'klm']:
        ...
        if strategy_lookback:
            ...
            print(f"  {strategy.upper()}: {len(strategy_symbols)} symbols, "
