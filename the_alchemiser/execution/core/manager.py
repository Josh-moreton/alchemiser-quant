        ...
        for order in orders_executed:
            ...
            if order:
                if isinstance(order, dict):
                    ...
                    order_id = getattr(order, "id", None) if hasattr(order, "id") else None
            ...
            try:
                ...
                if hasattr(self.engine, "portfolio_rebalancer") and hasattr(
                    ...
                    plan = RebalancePlan(
                        ...
                        trade_amount=(
                            item.trade_amount if item.action == "BUY" else -abs(item.trade_amount)
                        ),
                    ...
                    if order and order.get("order_id"):
                        ...
                        logging.error(f"❌ Failed to execute {item.action} {item.symbol}: {order}")
                else:
                    logging.error("❌ Engine missing portfolio_rebalancer.execution_service")
