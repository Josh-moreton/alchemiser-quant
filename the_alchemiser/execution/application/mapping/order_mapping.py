...
from the_alchemiser.execution.application.mapping.orders import normalize_order_status
...
def alpaca_order_to_domain(order: Any) -> Order:
    ...
    time_in_force_raw = get_attr("time_in_force") or "gtc"
    ...
    if time_in_force_str not in {"day", "gtc", "ioc", "fok"}:
        time_in_force_str = "gtc"  # Default fallback
    time_in_force = TimeInForce(cast(Literal["day", "gtc", "ioc", "fok"], time_in_force_str))
...
def raw_order_envelope_to_execution_result_dto(
    ...
    """
    from the_alchemiser.execution.application.mapping.alpaca_dto_mapping import (
