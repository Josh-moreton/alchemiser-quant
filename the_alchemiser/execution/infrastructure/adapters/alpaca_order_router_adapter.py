...
class AlpacaOrderRouterAdapter(OrderRouterPort):
    ...
    def __init__(self, api_key: str, secret_key: str, base_url: str) -> None:
        ...
        self._logger = logger.bind(component="AlpacaOrderRouterAdapter", base_url=base_url)
        ...
        try:
            ...
            if error_code == 40310000:  # Insufficient funds
                self._logger.warning(
                    "Insufficient funds for order", order_id=str(order.order_id), error=str(e)
                )
            ...
            if error_code in [40010001, 42210000]:  # Invalid symbols/parameters
                self._logger.warning(
                    "Invalid order parameters", order_id=str(order.order_id), error=str(e)
                )
                raise ValidationError(f"Invalid order parameters: {e}") from e
            self._logger.error(
            ...
            raise OrderExecutionError(f"Alpaca order submission failed: {e}") from e

        except Exception as e:
            self._logger.error(
                "Unexpected error submitting order", order_id=str(order.order_id), error=str(e)
            )
            raise OrderExecutionError(f"Unexpected error submitting order: {e}") from e
    ...
    def cancel_order(self, order_id: UUID) -> CancelAckVO:
        ...
        try:
            ...
            if "order not found" in str(e).lower():
                raise OrderNotFoundError(f"Order {order_id} not found at broker") from e
            self._logger.error(
                "Alpaca order cancellation failed", order_id=str(order_id), error=str(e)
            )
            raise OrderExecutionError(f"Alpaca order cancellation failed: {e}") from e
        except OrderNotFoundError:
            raise  # Re-raise domain exception as-is
        except Exception as e:
            self._logger.error(
                "Unexpected error cancelling order", order_id=str(order_id), error=str(e)
            )
    ...
    def get_order_status(self, order_id: UUID) -> OrderStatusVO:
        ...
        try:
            ...
            if "order not found" in str(e).lower():
                raise OrderNotFoundError(f"Order {order_id} not found at broker") from e
            self._logger.error(
                "Alpaca order status query failed", order_id=str(order_id), error=str(e)
            )
            raise OrderExecutionError(f"Alpaca order status query failed: {e}") from e
        except OrderNotFoundError:
            raise  # Re-raise domain exception as-is
        except Exception as e:
            self._logger.error(
                "Unexpected error getting order status", order_id=str(order_id), error=str(e)
            )
            raise OrderExecutionError(f"Unexpected error getting order status: {e}") from e
    ...
    def _find_broker_order_id(self, order_id: UUID) -> str:
        ...
        try:
            ...
            raise OrderExecutionError(f"Failed to lookup order {order_id}: {e}") from e
