...
class OrderExecutionParams:
    ...
    def __str__(self):
        return (f"OrderExecutionParams(wait={self.max_wait_seconds}s, "
...
class ProgressiveOrderCalculator:
    ...
    def calculate_volatility_metric(self, symbol: str, current_price: float, 
                                  recent_high: Optional[float] = None, recent_low: Optional[float] = None) -> float:
        """
    ...
    def calculate_execution_params(self, symbol: str, bid: float, ask: float, 
                                 side: OrderSide, urgency_level: str = "normal",
                                 recent_high: Optional[float] = None, recent_low: Optional[float] = None) -> OrderExecutionParams:
        """
        ...
        volatility = self.calculate_volatility_metric(symbol, current_price, recent_high, recent_low)
        spread_bps, spread_pct = self.calculate_spread_metric(bid, ask)
        
        logging.info(f"📊 Market conditions for {symbol}: volatility={volatility:.3f} ({volatility*100:.1f}%), "
    ...
    def calculate_step_price(self, bid: float, ask: float, side: OrderSide, 
                           step_percentage: float, tick_aggressiveness: float = 1.0) -> float:
        """
    ...
    def get_execution_strategy_description(self, params: OrderExecutionParams, 
                                         symbol: str, side: OrderSide) -> str:
        """
