...
class AlpacaMarketDataAdapter(MarketDataPort):
    ...
    def get_latest_bar(self, symbol: Symbol, timeframe: str) -> MarketBarVO:
        ...
        try:
            ...
            self._logger.info("Fetching latest bar", symbol=symbol.value, timeframe=timeframe)

            response = self._client.get_stock_bars(request)

            if symbol.value not in response.data or not response.data[symbol.value]:
                raise SymbolNotFoundError(f"No data returned for symbol: {symbol.value}")

            # Get the latest (only) bar
            alpaca_bar = response.data[symbol.value][0]

            # Convert via anti-corruption layer
            market_bar = self._mapper.alpaca_bar_to_market_bar_vo(alpaca_bar, symbol, timeframe)
            ...
            self._logger.error(
                "Failed to fetch latest bar", symbol=symbol.value, timeframe=timeframe, error=str(e)
            )
    ...
    def get_history(
        ...
        try:
            ...
            if symbol.value not in response.data or not response.data[symbol.value]:
                raise SymbolNotFoundError(f"No historical data for symbol: {symbol.value}")
