...
class SqsSignalPublisherAdapter(SignalPublisherPort):
    ...
    def __init__(self, queue_url: str, region_name: str = "us-east-1") -> None:
        ...
        self._logger = logger.bind(component="SqsSignalPublisherAdapter", queue_url=queue_url)
    ...
    def publish(self, signal: SignalContractV1) -> None:
        ...
        try:
            ...
            raise PublishError(f"Failed to publish signal to SQS: {e}") from e
            ...
            raise PublishError(f"Unexpected error publishing signal: {e}") from e
    ...
    def _validate_signal(self, signal: SignalContractV1) -> None:
        ...
        if signal.confidence < 0 or signal.confidence > 1:
            raise ValidationError(f"Signal confidence must be 0-1, got: {signal.confidence}")
    ...
    def _build_message_attributes(self, signal: SignalContractV1) -> dict[str, dict[str, str]]:
        ...
        """
        return {
            "MessageType": {"StringValue": "SignalV1", "DataType": "String"},
            "Symbol": {"StringValue": signal.symbol.value, "DataType": "String"},
            "Action": {"StringValue": signal.action.value, "DataType": "String"},
            "CorrelationId": {"StringValue": str(signal.correlation_id), "DataType": "String"},
            "SourceContext": {"StringValue": "Strategy", "DataType": "String"},
        }
