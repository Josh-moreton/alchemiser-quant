...
def handler(event: dict, context: Any) -> dict[str, Any]:
    ...
    logger.info(
        "Strategy signal handler invoked",
        extra={"correlation_id": correlation_id, "event_source": event.get("source", "unknown")},
    )
    ...
    try:
        ...
        logger.info(
            "Generating signals", extra={"symbol_count": len(symbols), "symbols": symbols_data}
        )
        ...
        logger.info(
            "Strategy signal generation completed",
            extra={
                "correlation_id": correlation_id,
