...
def _create_summary_page(result: BacktestResult) -> plt.Figure:
    ...
    left_metrics = [
        ...
        ("Initial Capital", f"${float(config.get('initial_capital', 0)):,.2f}"),
    ...
    fig.text(
        0.5,
        0.02,
        f"Generated by The Alchemiser Backtest Engine | {datetime.utcnow().strftime('%Y-%m-%d %H:%M UTC')}",
        ha="center",
        fontsize=8,
        color="gray",
    )
...
def _create_chart_page(
    ...
    **kwargs: dict,
    ...
    """
    # Get chart as bytes
    chart_bytes = chart_func(result, as_base64=False, **kwargs)
...
def generate_pdf_report(
    ...
    with PdfPages(str(output_path)) as pdf:
        ...
        if use_embedded_charts:
            ...
            monthly_fig = _create_chart_page(result, create_monthly_returns_heatmap, "Monthly Returns Heatmap")
            pdf.savefig(monthly_fig, bbox_inches="tight")
            plt.close(monthly_fig)

            # Page 5: Rolling Sharpe
            sharpe_fig = _create_chart_page(result, create_rolling_sharpe_chart, "Rolling Sharpe Ratio")
            pdf.savefig(sharpe_fig, bbox_inches="tight")
            plt.close(sharpe_fig)

            # Page 6: Trade Distribution
            dist_fig = _create_chart_page(result, create_trade_distribution_chart, "Trade Value Distribution")
