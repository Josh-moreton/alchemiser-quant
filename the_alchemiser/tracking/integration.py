...
class StrategyExecutionContext:
    ...
    @classmethod
    def record_order_if_active(cls, order_id: str, symbol: str, side: str, 
        ...
        if cls._current_strategy and cls._order_tracker and order_id:
            try:
                ...
                logging.info(f"Tracked {cls._current_strategy.value} order: {side} {quantity} {symbol} @ ${price:.2f}")
...
def track_order_execution(order_id: str, symbol: str, side: str, 
                         quantity: float, price: float) -> None:
    """Track an order execution in the current strategy context."""
    StrategyExecutionContext.record_order_if_active(
...
class StrategyTrackingMixin:
    ...
    def track_filled_order(self, order_id: str, symbol: str, side: str,
                          quantity: float, price: float) -> None:
    ...
    def get_strategy_pnl_summary(self, current_prices: Optional[Dict[str, float]] = None) -> Dict[str, Any]:
...
def extract_order_details_from_alpaca_order(order) -> Dict[str, Any]:
    ...
    try:
        ...
        price = float(
            getattr(order, 'filled_avg_price', 0) or 
            getattr(order, 'limit_price', 0) or 
            getattr(order, 'price', 0) or 
            0
        )
...
def configure_strategy_tracking_integration(trading_engine) -> None:
    ...
    try:
        ...
        if not hasattr(trading_engine, '_strategy_tracker'):
            ...
            trading_engine.get_strategy_pnl_summary = lambda prices=None: trading_engine._strategy_tracker.get_summary_for_email(prices)
