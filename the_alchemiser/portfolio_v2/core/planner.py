...
class RebalancePlanCalculator:
    ...
    def build_plan(
        ...
        try:
            ...
            trade_items = self._calculate_trade_items(target_values, current_values, snapshot.prices)
            
            # Ensure we have at least one item (required by DTO)
            if not trade_items:
                # Create a dummy HOLD item if no trades are needed
                dummy_symbol = list(strategy.target_weights.keys())[0] if strategy.target_weights else "CASH"
                trade_items = [RebalancePlanItemDTO(
                    symbol=dummy_symbol,
