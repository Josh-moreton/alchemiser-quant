...
class IndicatorValidationSuite:
    ...
    def fetch_twelvedata_indicator(self, symbol: str, indicator: str, period: int) -> Optional[float]:
    ...
    def validate_custom_indicators(self, symbol: str, price_data: pd.Series) -> List[Dict[str, Any]]:
        ...
        for period in self.indicator_definitions['moving_average_return']:
            ...
            passed = (latest_value is not None and 
                     -50 <= latest_value <= 50 and  # Reasonable range for MA returns
                     not np.isnan(latest_value) and 
                     not np.isinf(latest_value))
        ...
        for period in self.indicator_definitions['cumulative_return']:
            ...
            # Validate range and sanity
            passed = (latest_value is not None and 
                     -95 <= latest_value <= 1000 and  # Reasonable range for cumulative returns
                     not np.isnan(latest_value) and 
                     not np.isinf(latest_value))
    ...
    def validate_symbol(self, symbol: str) -> List[Dict[str, Any]]:
        ...
        for period in self.indicator_definitions['rsi']:
            ...
            self.console.print(f"  {status} RSI({period}): {result['our_value']} vs {result['twelvedata_value']}")
        ...
        for period in self.indicator_definitions['sma']:
            ...
            status = "✅" if result['passed'] else "❌"
            self.console.print(f"  {status} SMA({period}): {result['our_value']} vs {result['twelvedata_value']}")
    ...
    def run_validation_suite(self, symbols: List[str], mode: str = 'full') -> Dict[str, Any]:
        ...
        avg_calc_time = np.mean([r['calculation_time'] for r in all_results if r['calculation_time']])
    ...
    def generate_report(self, summary: Dict[str, Any]) -> None:
        ...
        if failed_results:
            ...
            for result in failed_results:
                self.console.print(f"  ❌ {result['symbol']} {result['indicator']}: {result['error']}")
