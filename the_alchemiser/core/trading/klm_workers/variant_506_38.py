...
class KLMVariant506_38(BaseKLMVariant):
    ...
    def _evaluate_single_popped_kmlm(self, indicators: Dict) -> Tuple[Union[str, Dict[str, float]], str, str]:
        """
    ...
    def _evaluate_bsc_strategy(self, indicators: Dict) -> Tuple[str, str, str]:
        ...
        if 'SPY' in indicators:
            ...
            if spy_rsi_21 > 30:
                result = ('VIXM', ActionType.BUY.value, "BSC: SPY RSI(21) > 30, moderate VIX position")
            else:
                result = ('SPXL', ActionType.BUY.value, "BSC: SPY oversold (RSI(21) <= 30), leveraged long position")
        else:
            # Fallback
    ...
    def _evaluate_combined_pop_bot(self, indicators: Dict) -> Tuple[Union[str, Dict[str, float]], str, str]:
        """
        ...
        if 'TQQQ' in indicators and indicators['TQQQ']['rsi_10'] < 30:
            result = ('TECL', ActionType.BUY.value, "Pop Bot: TQQQ oversold (RSI < 30), tech dip buy")
            self.log_decision(result[0], result[1], result[2])
            return result
        
        # Priority 2: SOXL oversold check
        if 'SOXL' in indicators and indicators['SOXL']['rsi_10'] < 30:
            result = ('SOXL', ActionType.BUY.value, "Pop Bot: SOXL oversold (RSI < 30), semiconductor dip buy")
            self.log_decision(result[0], result[1], result[2])
            return result
        
        # Priority 3: SPXL oversold check
        if 'SPXL' in indicators and indicators['SPXL']['rsi_10'] < 30:
            result = ('SPXL', ActionType.BUY.value, "Pop Bot: SPXL oversold (RSI < 30), S&P dip buy")
            self.log_decision(result[0], result[1], result[2])
            return result
        
        # Priority 4: LABU oversold check (biotech with different threshold)
        if 'LABU' in indicators and indicators['LABU']['rsi_10'] < 25:
            result = ('LABU', ActionType.BUY.value, "Pop Bot: LABU oversold (RSI < 25), biotech dip buy")
            self.log_decision(result[0], result[1], result[2])
            return result
    ...
    def evaluate_core_kmlm_switcher(self, indicators: Dict) -> Tuple[Union[str, Dict[str, float]], str, str]:
        ...
        if 'XLK' in indicators and 'KMLM' in indicators:
            ...
            if xlk_rsi > kmlm_rsi:
                ...
                result = ('FNGU', ActionType.BUY.value, f"KMLM Switcher: XLK RSI {xlk_rsi:.1f} > KMLM RSI {kmlm_rsi:.1f} → FNGU")
                self.log_decision(result[0], result[1], result[2])
                return result
    ...
    def _evaluate_long_short_rotator(self, indicators: Dict) -> Tuple[Union[str, Dict[str, float]], str, str]:
        """
        ...
        if volatility_candidates:
            ...
            result = ('KMLM', ActionType.BUY.value, "L/S Rotator: KMLM fallback (missing volatility data)")
    ...
    def get_required_symbols(self) -> list:
        ...
        overbought_symbols = ['QQQE', 'VTV', 'VOX', 'TECL', 'VOOG', 'VOOV', 'XLP', 'TQQQ', 'XLY', 'FAS', 'SPY']
        ...
        all_symbols = (overbought_symbols + single_popped_symbols + bsc_symbols + 
                      pop_bot_symbols + kmlm_switcher_symbols + rotator_symbols)
