...
class BaseKLMVariant(ABC):
    ...
    def apply_stdev_return_filter(self, candidates: list, indicators: Dict, window: int = 6) -> list:
    ...
    def evaluate_single_popped_kmlm(self, indicators: Dict) -> Tuple[Union[str, Dict[str, float]], str, str]:
        """
    ...
    def evaluate_bsc_strategy(self, indicators: Dict) -> Tuple[str, str, str]:
        ...
        if 'SPY' in indicators:
            ...
            if spy_rsi_21 > 30:
                result = ('VIXM', ActionType.BUY.value, f"BSC: SPY RSI(21) {spy_rsi_21:.1f} > 30 → VIXM")
            else:
                result = ('SPXL', ActionType.BUY.value, f"BSC: SPY RSI(21) {spy_rsi_21:.1f} ≤ 30 → SPXL")
        else:
            # Fallback
    ...
    def evaluate_combined_pop_bot(self, indicators: Dict) -> Tuple[Union[str, Dict[str, float]], str, str]:
        """
        ...
        if 'TQQQ' in indicators:
            ...
            if tqqq_rsi < 30:
                result = ('TECL', ActionType.BUY.value, f"Pop Bot: TQQQ RSI(10) {tqqq_rsi:.1f} < 30 → TECL")
                self.log_decision(result[0], result[1], result[2])
                return result
        ...
        if 'SOXL' in indicators:
            ...
            if soxl_rsi < 30:
                result = ('SOXL', ActionType.BUY.value, f"Pop Bot: SOXL RSI(10) {soxl_rsi:.1f} < 30 → SOXL")
                self.log_decision(result[0], result[1], result[2])
                return result
        ...
        if 'SPXL' in indicators:
            ...
            if spxl_rsi < 30:
                result = ('SPXL', ActionType.BUY.value, f"Pop Bot: SPXL RSI(10) {spxl_rsi:.1f} < 30 → SPXL")
                self.log_decision(result[0], result[1], result[2])
                return result
