...
class KLMVariant530_18(BaseKLMVariant):
    ...
    def evaluate(self, indicators: Dict[str, Dict[str, float]], 
                 market_data: Optional[Dict[str, pd.DataFrame]] = None) -> Tuple[Union[str, Dict[str, float]], str, str]:
        """
    ...
    def _evaluate_spy_scale_in(self, indicators: Dict) -> Optional[Tuple[Dict[str, float], str, str]]:
        ...
        if spy_rsi > 80:
            if spy_rsi > 82.5:
                ...
                result = (allocation, ActionType.BUY.value, f"SPY Scale-In: RSI {spy_rsi:.1f} > 82.5 → VIX Blend++")
            else:
                # VIX Blend+ - Equal VIX allocation
                allocation = self.VIX_BLEND_PLUS  # {UVXY: 0.333, VXX: 0.333, VIXM: 0.333}
                result = (allocation, ActionType.BUY.value, f"SPY Scale-In: RSI {spy_rsi:.1f} > 80 → VIX Blend+")
    ...
    def _evaluate_ioo_scale_in(self, indicators: Dict) -> Optional[Tuple[Dict[str, float], str, str]]:
        ...
        if ioo_rsi > 80:
            if ioo_rsi > 82.5:
                ...
                result = (allocation, ActionType.BUY.value, f"IOO Scale-In: RSI {ioo_rsi:.1f} > 82.5 → VIX Blend++")
            else:
                allocation = self.VIX_BLEND_PLUS
                result = (allocation, ActionType.BUY.value, f"IOO Scale-In: RSI {ioo_rsi:.1f} > 80 → VIX Blend+")
    ...
    def _evaluate_qqq_scale_in(self, indicators: Dict) -> Optional[Tuple[Dict[str, float], str, str]]:
        ...
        if qqq_rsi > 79:  # Different threshold!
            if qqq_rsi > 82.5:
                allocation = self.VIX_BLEND_PLUS_PLUS
                result = (allocation, ActionType.BUY.value, f"QQQ Scale-In: RSI {qqq_rsi:.1f} > 82.5 → VIX Blend++")
            else:
                allocation = self.VIX_BLEND_PLUS
                result = (allocation, ActionType.BUY.value, f"QQQ Scale-In: RSI {qqq_rsi:.1f} > 79 → VIX Blend+")
    ...
    def _evaluate_vtv_scale_in(self, indicators: Dict) -> Optional[Tuple[Dict[str, float], str, str]]:
        ...
        if vtv_rsi > 79:
            if vtv_rsi > 85:  # Higher threshold for VIX+
                allocation = self.VIX_BLEND_PLUS
                result = (allocation, ActionType.BUY.value, f"VTV Scale-In: RSI {vtv_rsi:.1f} > 85 → VIX Blend+")
            else:
                allocation = self.VIX_BLEND  # Uses VIXY instead of UVXY
                result = (allocation, ActionType.BUY.value, f"VTV Scale-In: RSI {vtv_rsi:.1f} > 79 → VIX Blend")
    ...
    def _evaluate_xlp_scale_in(self, indicators: Dict) -> Optional[Tuple[Dict[str, float], str, str]]:
        ...
        if xlp_rsi > 77:  # Different threshold
            if xlp_rsi > 85:
                allocation = self.VIX_BLEND_PLUS
                result = (allocation, ActionType.BUY.value, f"XLP Scale-In: RSI {xlp_rsi:.1f} > 85 → VIX Blend+")
            else:
                allocation = self.VIX_BLEND
                result = (allocation, ActionType.BUY.value, f"XLP Scale-In: RSI {xlp_rsi:.1f} > 77 → VIX Blend")
    ...
    def _evaluate_xlf_scale_in(self, indicators: Dict) -> Optional[Tuple[Dict[str, float], str, str]]:
        ...
        if xlf_rsi > 81:
            if xlf_rsi > 85:
                allocation = self.VIX_BLEND_PLUS
                result = (allocation, ActionType.BUY.value, f"XLF Scale-In: RSI {xlf_rsi:.1f} > 85 → VIX Blend+")
            else:
                allocation = self.VIX_BLEND
                result = (allocation, ActionType.BUY.value, f"XLF Scale-In: RSI {xlf_rsi:.1f} > 81 → VIX Blend")
    ...
    def _evaluate_retl_scale_in(self, indicators: Dict) -> Optional[Tuple[Dict[str, float], str, str]]:
        ...
        if retl_rsi > 82:
            if retl_rsi > 85:
                allocation = self.VIX_BLEND
                result = (allocation, ActionType.BUY.value, f"RETL Scale-In: RSI {retl_rsi:.1f} > 85 → VIX Blend")
            else:
                allocation = self.BTAL_BIL  # {BTAL: 0.5, BIL: 0.5}
                result = (allocation, ActionType.BUY.value, f"RETL Scale-In: RSI {retl_rsi:.1f} > 82 → BTAL/BIL")
    ...
    def _evaluate_spy_rsi_70_overbought_logic(self, indicators: Dict) -> Optional[Tuple[Union[str, Dict[str, float]], str, str]]:
        """
        ...
        if spy_rsi_70 > 63:
            ...
            if agg_rsi_15 > qqq_rsi_15:
                ...
                allocation = {
                    'TQQQ': 0.2, 'SPXL': 0.2, 'SOXL': 0.2, 'FNGU': 0.2, 'ERX': 0.2
                }
                result = (allocation, ActionType.BUY.value, 
    ...
    def _evaluate_holy_grail_kmlm_switcher(self, indicators: Dict) -> Tuple[Union[str, Dict[str, float]], str, str]:
        """
        ...
        if 'VOX' in indicators and indicators['VOX'].get('rsi_10', 0) > 79:
            allocation = self.VIX_BLEND
            result = (allocation, ActionType.BUY.value, f"Holy Grail: VOX RSI(10) > 79 → VIX Blend")
            self.log_decision(result[0], result[1], result[2])
            return result
        ...
        if 'XLP' in indicators and indicators['XLP'].get('rsi_10', 0) > 75:
            allocation = self.VIX_BLEND
            result = (allocation, ActionType.BUY.value, f"Holy Grail: XLP RSI(10) > 75 → VIX Blend")
            self.log_decision(result[0], result[1], result[2])
            return result
    ...
    def _evaluate_holy_grail_pop_bot(self, indicators: Dict) -> Tuple[Union[str, Dict[str, float]], str, str]:
    ...
    def _evaluate_kmlm_switcher_plus_fngu(self, indicators: Dict) -> Tuple[Union[str, Dict[str, float]], str, str]:
        """
        ...
        if xlk_rsi > kmlm_rsi:
            ...
            if candidates[0][0] == "FNGU_COMPLEX":
                ...
                if comparison_candidates:
                    ...
                    if best_candidate[0] != "FNGU":
                        ...
                        result = ("FNGU", ActionType.BUY.value, "KMLM Switcher: 100% FNGU (best MA return)")
                else:
                    result = ("FNGU", ActionType.BUY.value, "KMLM Switcher: FNGU fallback")
            else:
                # Simple tech selection
                result = (candidates[0][0], ActionType.BUY.value, 
            ...
            if len(rotator_candidates) >= 3:
                ...
                result = (allocation, ActionType.BUY.value, f"L/S Rotator: {symbols} (lowest volatility)")
            elif rotator_candidates:
                # Less than 3 available
                best = min(rotator_candidates, key=lambda x: x[1])
                result = (best[0], ActionType.BUY.value, f"L/S Rotator: {best[0]} (lowest volatility)")
            else:
                # Ultimate fallback
    ...
    def get_required_symbols(self) -> list:
        ...
        all_symbols = (scale_in_symbols + vix_symbols + defensive_symbols + 
                      overbought_symbols + tech_3x_symbols + commodity_symbols +
                      holy_grail_symbols + kmlm_switcher_symbols + additional_symbols)
        
        return list(set(all_symbols))
    
    # Override the base class method since 530/18 doesn't use standard pattern
    def evaluate_core_kmlm_switcher(self, indicators: Dict) -> Tuple[Union[str, Dict[str, float]], str, str]:
        """
