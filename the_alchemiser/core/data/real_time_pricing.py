...
class RealTimePricingService:
    ...
    def __init__(self, api_key: str, secret_key: str, paper_trading: bool = True):
        ...
        logging.info(f"📡 Real-time pricing service initialized ({'paper' if paper_trading else 'live'})")
    ...
    async def _on_quote(self, quote: Union[Quote, Dict[str, Any]]) -> None:
        ...
        try:
            ...
            logging.debug(f"📊 Quote: {symbol} ${float(bid_price or 0):.2f}/${float(ask_price or 0):.2f}")
            
        except Exception as e:
            symbol_str = str(quote.get('symbol', 'unknown') if isinstance(quote, dict) else getattr(quote, 'symbol', 'unknown'))
    ...
    async def _on_trade(self, trade: Union[Trade, Dict[str, Any]]) -> None:
        ...
        try:
            ...
            symbol_str = str(trade.get('symbol', 'unknown') if isinstance(trade, dict) else getattr(trade, 'symbol', 'unknown'))
        ...
        return {
            **self._stats,
            ...
            'uptime_seconds': (
                ...
                if self._stats['last_heartbeat'] else 0
            )
        }
    ...
    def subscribe_symbol(self, symbol: str, priority: Optional[float] = None) -> None:
        ...
        with self._subscription_lock:
            ...
            if symbol not in self._subscribed_symbols and len(self._subscribed_symbols) >= self._max_symbols:
                ...
                if priority > lowest_priority:
                    ...
                    logging.info(f"📊 Subscription limit reached. Replacing {lowest_priority_symbol} (priority: {lowest_priority:.1f}) with {symbol} (priority: {priority:.1f})")
                    self.unsubscribe_symbol(lowest_priority_symbol)
                    self._stats['subscription_limit_hits'] += 1
                else:
                    logging.warning(f"⚠️ Cannot subscribe to {symbol} - priority {priority:.1f} too low (limit: {self._max_symbols} symbols)")
                    return
            ...
            if symbol not in self._subscribed_symbols:
                ...
                if self._stream and self._connected:
                    try:
                        ...
                        logging.info(f"📡 Subscribed to real-time data for {symbol} (priority: {priority:.1f})")
                    except Exception as e:
                ...
                self._subscription_priority[symbol] = max(
                    self._subscription_priority.get(symbol, 0), 
                    priority
                )
    ...
    def subscribe_for_order_placement(self, symbol: str) -> None:
        ...
        logging.debug(f"Subscribed to {symbol} for order placement (priority: {order_priority:.1f})")
        ...
        with self._subscription_lock:
            if symbol in self._subscribed_symbols:
                ...
                if current_priority > base_priority + 86400:  # More than 1 day ahead
                    ...
                    logging.debug(f"Keeping subscription for {symbol} (normal priority: {current_priority:.1f})")
