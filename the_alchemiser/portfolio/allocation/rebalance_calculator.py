...
class RebalanceCalculator:
    ...
    def calculate_rebalance_plan(
        ...
        logger.info(
            f"PORTFOLIO_VALUE_VS_CURRENT_SUM: ${portfolio_value_float:.2f} vs ${total_current_values:.2f}"
        )
        ...
        for symbol in target_weights_float:
            ...
            current_weight = (
                current_value / portfolio_value_float if portfolio_value_float > 0 else 0.0
            )
            ...
            if abs(weight_diff) < threshold_float:
                logger.warning(
                    f"⚠️ {symbol} BELOW_THRESHOLD: diff={abs(weight_diff):.6f} < threshold={threshold_float:.6f}"
                )
            else:
                logger.info(
                    f"✅ {symbol} ABOVE_THRESHOLD: diff={abs(weight_diff):.6f} >= threshold={threshold_float:.6f}"
                )
