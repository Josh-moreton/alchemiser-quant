...
class DynamoDbPositionRepositoryAdapter(PositionRepositoryPort):
    ...
    def __init__(self, table_name: str, region_name: str = "us-east-1") -> None:
        ...
        self._logger = logger.bind(
            component="DynamoDbPositionRepositoryAdapter", table_name=table_name
        )
    ...
    def load_positions(self) -> Sequence[Position]:
        ...
        try:
            ...
            self._logger.debug("Successfully loaded positions", position_count=len(positions))
            ...
            raise DataAccessError(f"Failed to load positions from DynamoDB: {e}") from e
    ...
    def save_positions(self, positions: Sequence[Position]) -> None:
        ...
        try:
            ...
            self._logger.info("Saving positions to DynamoDB", position_count=len(positions))
            ...
            if error_code == "ConditionalCheckFailedException":
                self._logger.warning("Optimistic lock violation saving positions", error=str(e))
                raise ConcurrencyError("Position was modified by another process") from e
            self._logger.error(
                "DynamoDB error saving positions", error=str(e), error_code=error_code
            )
            raise DataAccessError(f"Failed to save positions to DynamoDB: {e}") from e
    ...
    def get_position(self, symbol: Symbol) -> Position | None:
        ...
        """
        try:
            self._logger.debug("Getting position by symbol", symbol=symbol.value)

            response = self._table.get_item(Key={"symbol": symbol.value})

            if "Item" not in response:
                self._logger.debug("Position not found", symbol=symbol.value)
                return None
            ...
            raise DataAccessError(f"Failed to get position for {symbol.value}: {e}") from e
