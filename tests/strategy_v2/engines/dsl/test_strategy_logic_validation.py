...
class TestStrategyLogicExecution:
    ...
    def test_nuclear_strategy_rsi_conditions(self, repository_root: Path) -> None:
        ...
        assert "UVXY" in result.allocation_result.target_weights, "High RSI should trigger UVXY"
        ...
        assert result.allocation_result.target_weights != result2.allocation_result.target_weights, \
    ...
    def test_nuclear_rsi_boundaries(
        ...
        assert result.success, f"Nuclear should evaluate successfully with RSI {rsi_value}"
        assert result.allocation_result is not None, "Should produce allocation"
        
        allocations = result.allocation_result.target_weights
        
        if expected_behavior == "defensive":
            assert "UVXY" in allocations, \
            ...
            allocated_stocks = set(allocations.keys())
            assert allocated_stocks.intersection(aggressive_stocks), \
    ...
    def test_tecl_strategy_weight_allocations(self, repository_root: Path) -> None:
        ...
        assert "UVXY" in allocations or "BIL" in allocations, \
    ...
    def test_strategy_allocation_weights_sum_to_one(self, repository_root: Path) -> None:
        ...
        for strategy_file in strategies_to_test:
            ...
            assert result.allocation_result is not None, f"{strategy_file} should produce allocation"
            
            total_weight = sum(result.allocation_result.target_weights.values())
            assert abs(float(total_weight) - 1.0) < 0.001, \
    ...
    def test_defensive_vs_aggressive_allocations(self, repository_root: Path) -> None:
        ...
        for strategy_file in strategies_to_test:
            ...
            if defensive_result.success and aggressive_result.success:
                if defensive_result.allocation_result and aggressive_result.allocation_result:
                    def_allocs = set(defensive_result.allocation_result.target_weights.keys())
                    agg_allocs = set(aggressive_result.allocation_result.target_weights.keys())
                    
                    # Check if allocations change between defensive and aggressive conditions
                    assert def_allocs != agg_allocs or \
                           defensive_result.allocation_result.target_weights != aggressive_result.allocation_result.target_weights, \
...
class TestStrategyScenarioMatrix:
    ...
    @pytest.mark.parametrize("strategy_file", [
        "Nuclear.clj", "TECL.clj", "KLM.clj", "Grail.clj"
    ])
    @pytest.mark.parametrize("scenario_type,rsi_range", [
    ...
    def test_cross_scenario_allocation_differences(self, repository_root: Path) -> None:
        ...
        for strategy_file in strategies:
            ...
            for scenario_name, rsi_value in scenarios:
                ...
                if result.success and result.allocation_result:
                    scenario_results[scenario_name] = result.allocation_result.target_weights
            ...
            all_same = all(allocations_list[0] == alloc for alloc in allocations_list[1:])
