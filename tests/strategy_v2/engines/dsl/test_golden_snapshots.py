...
class TestGoldenSnapshots:
        ...
        for strategy_file in clj_strategy_files:
            ...
            try:
                ...
                if golden_data is None:
                    ...
                    pytest.skip(
                        f"Created initial snapshot for {strategy_name} - run again to validate"
                    )
                else:
    ...
    def _compare_snapshots(
        ...
        assert (
            ...
            == expected["request"]["strategy_config_path"]
        )
    ...
    def _validate_allocation_correctness(
        self, allocation_data: dict, strategy_name: str
    ) -> None:
        ...
        if "allocations" in allocation_data:
            ...
            assert len(allocations) > 0, f"{strategy_name} has empty allocation"
    ...
    def test_regenerate_snapshots_if_requested(
        ...
        if not os.getenv("REGENERATE_SNAPSHOTS"):
            pytest.skip(
                "Snapshot regeneration not requested (set REGENERATE_SNAPSHOTS=1)"
            )
...
class TestSnapshotUtilities:
    ...
    def test_snapshot_serialization(
        self, repository_root: Path, clj_strategy_files: list[Path], tmp_path: Path
    ) -> None:
    ...
    def test_snapshot_diff_detection(
        ...
        assert (
            snapshot1["summary"]["total_events"] == snapshot2["summary"]["total_events"]
        )
        assert (
            snapshot1["summary"]["event_counts"] == snapshot2["summary"]["event_counts"]
        )
        ...
        assert (
            snapshot1["request"]["correlation_id"]
            != snapshot3["request"]["correlation_id"]
        )
    ...
    def test_full_pipeline_snapshots(
        ...
        for strategy_file in test_strategies:
            ...
            try:
                ...
                pipeline_results[strategy_name] = {"error": str(e)}
                ...
                if strategy_name in existing_pipeline:
                    ...
                    if "error" not in current_result and "error" not in expected_result:
                        ...
                        if current_result["events"] != expected_result["events"]:
                            print(
                                f"Pipeline event count changed for {strategy_name}: "
                                f"{expected_result['events']} -> {current_result['events']}"
                            )

                        if current_result["success"] != expected_result["success"]:
                            print(
                                f"Pipeline success status changed for {strategy_name}: "
