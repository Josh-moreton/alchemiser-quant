...
class TestEventDrivenWorkflow:
    ...
    def test_end_to_end_event_flow(
        ...
        assert any(
            event.event_type == "StrategyEvaluationRequested"
            for event in result.all_events
        )
    ...
    def test_event_bus_metrics(
        ...
        assert (
            final_stats["total_events_published"]
            > initial_stats["total_events_published"]
        )
    ...
    def test_multiple_evaluations_same_harness(
        ...
        if len(clj_strategy_files) < 2:
            pytest.skip(
                "Need at least 2 strategy files for multiple evaluation testing"
            )
        ...
        for i, strategy_file in enumerate(clj_strategy_files[:3]):  # Test first 3
            ...
            print(
                f"Evaluation {i+1} ({strategy_file.name}): {len(result.all_events)} events"
            )
