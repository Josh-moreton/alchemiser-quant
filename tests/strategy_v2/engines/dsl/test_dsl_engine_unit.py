...
class TestDslEngine:
    ...
    def test_engine_initialization(self, repository_root: Path) -> None:
        ...
        strategies_path = (
            repository_root / "the_alchemiser" / "strategy_v2" / "strategies"
        )
        engine = DslEngine(strategy_config_path=str(strategies_path))
        assert engine is not None
        assert engine.strategy_config_path == str(strategies_path)
    ...
    def test_engine_direct_evaluation(
        ...
        try:
            allocation, trace = engine.evaluate_strategy(strategy_file.name)
...
class TestDslEngineEventHandling:
    ...
    def test_handle_evaluation_request_event(
        self, repository_root: Path, clj_strategy_files: list[Path], event_recorder
    ) -> None:
        ...
        assert (
            result.success or len(result.all_events) > 0
        )  # Either success or events generated

        # Check that request was processed
        request_events = [
            e
            for e in result.all_events
            if e.event_type == "StrategyEvaluationRequested"
        ]
    ...
    def test_event_correlation_id_propagation(
        ...
        result = harness.evaluate_strategy(
            str(strategy_file), correlation_id=correlation_id
        )
...
class TestDslEngineErrorHandling:
    ...
    def test_malformed_strategy_file(
...
class TestDslEngineIntegration:
    ...
    def test_strategy_evaluation_produces_results(
        ...
        print(
            f"Successfully evaluated {successful_evaluations}/{len(clj_strategy_files)} strategies"
        )
