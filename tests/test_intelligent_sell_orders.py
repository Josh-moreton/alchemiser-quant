    ...
    try:
        ...
        for symbol in test_symbols:
            ...
            if bid > 0 and ask > 0 and current_price:
                ...
                if improvement > 0:
                    print(
                        f"   💰 Potential improvement: +${improvement:.2f} (+{improvement_bps:.1f} bps)"
                    )
                else:
                    print(
                        f"   ⚡ Speed vs profit tradeoff: {improvement:.2f} (-{abs(improvement_bps):.1f} bps for faster fill)"
                    )
                ...
                if sell_distance_from_mid < buy_distance_from_mid:
                    print("   ✅ Sells are MORE aggressive than buys (favors quick fills)")
                else:
                    print("   ⚠️ Sells are less aggressive than buys")

            else:
                print(f"   ❌ Could not get valid quote data for {symbol}")

        print("\n🔄 Sell Order Strategy Summary:")
        print("   • Uses WebSocket real-time bid/ask pricing")
        print("   • Places limit 85% toward bid from ask (more aggressive than 75% for buys)")
        print("   • Waits only 10 seconds vs 15 seconds for buys")
        print("   • Falls back to market order if limit doesn't fill")
        print("   • Prioritizes quick fills over maximum profit")
        print("   • Still better than pure market orders in most cases")
