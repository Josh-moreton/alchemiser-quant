        ...
        for i, symbol in enumerate(test_symbols, 1):
            ...
            if hasattr(data_provider, "get_current_price_for_order"):
                ...
                if len(after_cleanup) <= len(initial_subs):
                    print("   ✅ Cleanup effective (reduced or maintained subscription count)")
                else:
                    print("   ⚠️ Subscriptions increased (expected in paper trading due to limits)")
...
def demonstrate_scalability():
    ...
    many_symbols = ["AAPL", "MSFT", "GOOGL", "AMZN", "TSLA", "META", "NVDA", "NFLX", "UBER", "ZOOM"]
