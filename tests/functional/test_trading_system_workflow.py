...
class TestTradingSystemWorkflow:
    ...
    @patch(
        "the_alchemiser.shared.config.container.ApplicationContainer.create_for_testing"
    )
    def test_trading_system_initialization(
        self, mock_container_factory, mock_container
    ):
    ...
    @patch(
        "the_alchemiser.orchestration.system.TradingSystem._initialize_event_driven_orchestrator"
    )
    @patch(
        "the_alchemiser.shared.config.container.ApplicationContainer.create_for_testing"
    )
    def test_trading_execution_with_mocked_dependencies(
        self, mock_container_factory, mock_orchestrator_init, mock_container
    ):
