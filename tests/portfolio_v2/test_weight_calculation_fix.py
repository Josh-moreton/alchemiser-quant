...
class TestRebalancePlanCalculatorWeightFix:
    ...
    def test_sell_partial_position_not_liquidation(self, calculator):
        ...
        assert abs(aapl_item.current_weight - Decimal("0.101")) < Decimal("0.01"), "Current weight should be ~10.1% (accounting for cash reserve)"
        ...
        assert abs(aapl_item.trade_amount - Decimal("-505")) < Decimal("10"), "Should sell approximately $505 worth"
    ...
    def test_full_liquidation_still_works(self, calculator):
        ...
        assert abs(aapl_item.trade_amount - Decimal("-1000")) < Decimal("0.01"), "Should liquidate full $1000 position"
    ...
    def test_buy_position_weights_calculated(self, calculator):
        ...
        assert abs(aapl_item.current_weight - Decimal("0.0505")) < Decimal("0.01"), "Current weight should be ~5.05%"
    ...
    def test_weight_diff_calculation(self, calculator):
        ...
        assert abs(aapl_item.weight_diff - expected_weight_diff) < Decimal("0.0001"), "weight_diff should equal target_weight - current_weight"
        
        # Since target (5%) < current (~10%), weight_diff should be negative
        assert aapl_item.weight_diff < Decimal("0"), "weight_diff should be negative when reducing position"