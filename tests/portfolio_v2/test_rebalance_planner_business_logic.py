...
class TestRebalancePlanCalculator:
    ...
    def test_rebalance_plan_basic_calculation(self, calculator, sample_strategy_allocation, sample_portfolio_snapshot):
        ...
        for item in plan.items:
            ...
            assert item.target_allocation_pct is not None
            assert item.action in ["BUY", "SELL", "HOLD"]

    def test_rebalance_from_empty_portfolio(self, calculator, sample_strategy_allocation, empty_portfolio_snapshot):
        ...
        for item in plan.items:
            if item.target_value > Decimal("0"):
                ...
                assert item.target_quantity > Decimal("0")

    def test_target_value_calculations(self, calculator, sample_strategy_allocation, sample_portfolio_snapshot):
    ...
    def test_rebalance_determines_correct_actions(self, calculator, sample_portfolio_snapshot):
        ...
        for item in plan.items:
            ...
            current_allocation_pct = current_value / sample_portfolio_snapshot.total_value if sample_portfolio_snapshot.total_value > 0 else Decimal("0")
    ...
    def test_plan_preserves_correlation_metadata(self, calculator, sample_strategy_allocation, sample_portfolio_snapshot):
    ...
    def test_cash_management_calculations(self, calculator, sample_strategy_allocation, sample_portfolio_snapshot):
        ...
        total_buy_value = sum(item.target_value for item in plan.items if item.action == "BUY")
        total_sell_value = sum(abs(item.target_value - (item.current_quantity * item.target_value / item.target_quantity if item.target_quantity > 0 else 0)) 
                              for item in plan.items if item.action == "SELL")
