...
class TestTradingEngineUnit:
    ...
    def test_init_paper_trading(self, mock_config):
        ...
        with (
            patch("the_alchemiser.core.config.load_settings", return_value=mock_config),
    ...
    def test_init_with_custom_allocations(self, mock_config):
        ...
        custom_allocations = {StrategyType.NUCLEAR: 0.7, StrategyType.TECL: 0.3}

        with (
            patch("the_alchemiser.core.config.get_config", return_value=mock_config),
            patch("the_alchemiser.execution.trading_engine.UnifiedDataProvider"),
            patch("the_alchemiser.execution.trading_engine.SmartExecution"),
            patch("the_alchemiser.execution.trading_engine.PortfolioRebalancer"),
            patch("the_alchemiser.execution.trading_engine.MultiStrategyManager") as mock_sm,
        ):
            engine = TradingEngine(paper_trading=True, strategy_allocations=custom_allocations)
    ...
    def test_get_account_info_success(self, trading_engine):
        ...
        with patch(
            "the_alchemiser.utils.account_utils.extract_comprehensive_account_data",
    ...
    def test_get_account_info_failure(self, trading_engine):
        ...
        with patch(
            "the_alchemiser.utils.account_utils.extract_comprehensive_account_data", return_value={}
        ) as mock_extract:
            result = trading_engine.get_account_info()
    ...
    def test_execute_rebalancing_compatibility(self, trading_engine):
        ...
        expected = {
            "trading_summary": {"total_orders": 1, "orders_executed": mock_orders},
            "orders": mock_orders,
        }
        assert result == expected
    ...
    def test_get_multi_strategy_performance_report_success(self, trading_engine):
        ...
        trading_engine.data_provider.get_positions.return_value = [{"symbol": "SPY", "qty": 10}]
        trading_engine.strategy_manager.strategy_allocations = {
            StrategyType.NUCLEAR: 0.6,
            StrategyType.TECL: 0.4,
        }
        trading_engine.strategy_manager.get_strategy_performance_summary.return_value = (
            mock_performance
        )
    ...
    def test_build_portfolio_state_data(
        self, mock_calc_discrepancy, mock_extract_values, mock_calc_values, trading_engine
    ):
