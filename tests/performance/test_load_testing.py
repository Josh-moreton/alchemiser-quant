...
class TestMarketDataLoad:
    ...
    def test_sustained_trading_load(self):
        ...
        with ThreadPoolExecutor(max_workers=num_workers) as executor:
            futures = [
                executor.submit(trading_worker, i, test_duration) 
        ...
        assert load_tester.metrics.throughput_per_second > target_orders_per_second * 0.8, \
            f"Order throughput too low: {load_tester.metrics.throughput_per_second:.0f}/sec"
        
        assert load_tester.metrics.error_rate < 0.05, \
            f"Error rate too high: {load_tester.metrics.error_rate:.2%}"
        
        assert load_tester.get_percentile_response_time(99) < 0.1, \
        ...
        assert trading_engine.orders_filled > total_orders * 0.9, \
            "Order fill rate too low"
        
        assert trading_engine.errors < total_orders * 0.02, \
            "Too many trading engine errors"
    ...
    def test_portfolio_calculation_load(self):
            ...
            def calculate_portfolio_metrics(self, positions: dict, market_prices: dict) -> dict:
                ...
                try:
                    ...
                    for symbol in position_values:
                        if total_value > 0:
                            position_values[symbol]['weight'] = \
                                float(position_values[symbol]['market_value'] / total_value)
                    ...
                    return {'error': 'calculation_failed', 'calculation_time': time.time() - start_time}
        ...
        with ThreadPoolExecutor(max_workers=num_workers) as executor:
            futures = [
                executor.submit(calculation_worker, i, test_duration)
                for i in range(num_workers)
            ]
...
class TestScalabilityLimits:
    ...
    def test_memory_scaling_with_data_volume(self):
        ...
        def process_large_dataset(num_symbols: int, days: int) -> dict:
            ...
            for symbol_idx in range(num_symbols):
                ...
                for day in range(days):
                    ...
                    for minute in range(390):  # 6.5 hour trading day
                        ...
                        data_points.append({
                            'symbol': symbol,
                            'timestamp': f'2024-01-01 09:30:00',  # Simplified timestamp
            ...
            df['volume_ma'] = df.groupby('symbol')['volume'].rolling(20).mean().reset_index(0, drop=True)
            df['price_ma'] = df.groupby('symbol')['price'].rolling(50).mean().reset_index(0, drop=True)
        ...
        for test_case in test_cases:
            ...
            memory_measurements.append({
                ...
                'memory_per_point_kb': (memory_used * 1024) / data_points if data_points > 0 else 0,
                'throughput_points_per_sec': data_points / processing_time if processing_time > 0 else 0
            })
        
        # Validate memory scaling
        for measurement in memory_measurements:
            # Memory per data point should be reasonable
            assert measurement['memory_per_point_kb'] < 5.0, \
                f"Memory usage too high: {measurement['memory_per_point_kb']:.2f} KB/point"
            
            # Processing should be reasonable
            assert measurement['throughput_points_per_sec'] > 1000, \
        ...
        if len(memory_measurements) >= 2:
            ...
            assert efficiency_ratio < 3.0, f"Memory efficiency degrades too much: {efficiency_ratio:.2f}x"
