...
class TestTechnicalIndicators(unittest.TestCase):
    ...
    def test_rsi_calculation_accuracy(self):
        ...
        for window in test_windows:
            with self.subTest(window=window):
                ...
                self.assertTrue((valid_rsi >= 0).all(), f"RSI values below 0 found for window {window}")
                self.assertTrue((valid_rsi <= 100).all(), f"RSI values above 100 found for window {window}")
                ...
                if len(stable_rsi) > 0:
                    ...
                    self.assertTrue(stable_rsi.std() > 1, f"RSI shows insufficient variation for window {window}")
                ...
                if len(rsi_up.dropna()) > 0:
                    ...
                    self.assertGreater(final_rsi_up, 45, f"RSI should be elevated in uptrend for window {window}")
                
                # In downtrend, final RSI should be < 50 (if enough data)
                if len(rsi_down.dropna()) > 0:
                    final_rsi_down = rsi_down.dropna().iloc[-1]
                    self.assertLess(final_rsi_down, 55, f"RSI should be depressed in downtrend for window {window}")
    ...
    def test_rsi_wilder_smoothing_accuracy(self):
        ...
        test_data = pd.Series([44.0, 44.3, 44.1, 44.2, 44.5, 43.9, 44.5, 44.9, 44.5, 44.6, 
                              44.8, 44.2, 45.1, 46.0, 47.0, 46.5, 46.8, 47.2, 46.9, 47.5])
    ...
    def test_moving_average_calculation(self):
        ...
        for window in test_windows:
            with self.subTest(window=window):
                ...
                self.assertTrue(ma.iloc[:window-1].isna().all(), 
                               f"First {window-1} MA values should be NaN")
                
                # Values from window onward should be valid
                valid_ma = ma.iloc[window-1:]
                self.assertTrue(valid_ma.notna().all(), 
                ...
                if len(self.sample_data) >= window:
                    ...
                    self.assertAlmostEqual(expected_first_ma, actual_first_ma, places=6,
                    ...
                    for i in range(window, min(window + 5, len(self.sample_data))):
                        ...
                        actual_ma = ma.iloc[i]
                        self.assertAlmostEqual(expected_ma, actual_ma, places=6,
    ...
    def test_external_validation_spy_rsi(self):
        ...
        try:
            ...
            history_url = f"https://api.twelvedata.com/time_series"
            history_params = {
                'symbol': symbol,
                'interval': '1day',
                'outputsize': '50',
            ...
            print(f"✅ RSI Validation {symbol}: Ours={our_latest_rsi:.2f}, "
                  f"TwelveData={twelvedata_rsi:.2f}, Diff={difference:.2f}")
    ...
    def test_strategy_specific_indicators(self):
        ...
        for indicator_name, indicator_func in nuclear_indicators.items():
            with self.subTest(strategy="Nuclear", indicator=indicator_name):
                ...
                self.assertIsInstance(result, pd.Series, f"Nuclear {indicator_name} should return Series")
                self.assertEqual(len(result), len(self.sample_data), f"Nuclear {indicator_name} length mismatch")
        ...
        for indicator_name, indicator_func in tecl_indicators.items():
            with self.subTest(strategy="TECL", indicator=indicator_name):
                result = indicator_func(self.sample_data)
                self.assertIsInstance(result, pd.Series, f"TECL {indicator_name} should return Series")
                self.assertEqual(len(result), len(self.sample_data), f"TECL {indicator_name} length mismatch")
    ...
    def test_indicator_consistency_across_windows(self):
        ...
        for window, ma in ma_values.items():
            if len(ma) > 0:
                ...
                self.assertLess(relative_diff, 0.5, f"MA {window} deviates too much from price data")
