...
class TestOrderExecutionLogging:
    ...
    def test_order_settlement_logging(self, order_manager, mock_trading_client):
        ...
        with patch("logging.info") as mock_info:
            ...
            result = order_manager.wait_for_settlement(
                sell_orders, max_wait_time=1, poll_interval=1
            )
...
class TestErrorLogging:
    ...
    def test_network_error_logging(self, order_manager, mock_trading_client):
        ...
        # Mock network error
        mock_trading_client.submit_order.side_effect = requests.exceptions.ConnectionError(
            "Network timeout"
        )
...
class TestPortfolioStateReporting:
    ...
    def test_position_change_tracking(self, order_manager, mock_trading_client):
        ...
        initial_positions = [MagicMock(symbol="AAPL", qty=10.0, market_value=1500.0)]
        ...
        mock_trading_client.get_all_positions.side_effect = [initial_positions, updated_positions]
    ...
    def test_daily_performance_reporting(self, order_manager, mock_trading_client):
        ...
        morning_account = MagicMock(
            portfolio_value=10000.0, day_trading_buying_power=40000.0, buying_power=20000.0
        )
    ...
    def test_risk_metrics_reporting(self, order_manager, mock_trading_client):
        ...
        mock_trading_client.get_account.return_value = MagicMock(
            portfolio_value=25000.0, buying_power=5000.0
        )
        ...
        position_weights = {
            pos.symbol: pos.market_value / account.portfolio_value for pos in positions
        }
...
class TestAuditTrail:
    ...
    def test_complete_order_lifecycle_audit(self, order_manager, mock_trading_client):
        ...
        def capture_log(message, *args, **kwargs):
            order_events.append(
                {"timestamp": datetime.now().isoformat(), "message": str(message), "level": "INFO"}
            )
