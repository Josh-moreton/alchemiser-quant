...
class MockMarketDataService:
    ...
    def __init__(self, seed: int = 42) -> None:
        ...
        self._custom_indicators: dict[tuple[str, str], float] = (
            {}
    ...
    def _generate_indicator_value(
        ...
        if indicator_type.lower() in ["sma", "ema"]:
...
class StrategyDiscovery:
    ...
    def discover_clj_files(self) -> list[Path]:
        ...
        strategy_dir = (
            self.repository_root / "the_alchemiser" / "strategy_v2" / "strategies"
        )
...
class ScenarioGenerator:
    ...
    def generate_multi_symbol_scenario(self) -> dict[str, Any]:
        ...
        return {
            ...
            "symbols": symbols,
            "price_changes": {
                symbol: self._random.uniform(-0.05, 0.05) for symbol in symbols
            },
...
def assert_events_match_golden(
    ...
    assert len(actual_events) == len(
        golden_events
    ), f"Event count mismatch: expected {len(golden_events)}, got {len(actual_events)}"

    for i, (actual, expected) in enumerate(zip(actual_events, golden_events, strict=False)):
        # Compare event types
