...
def test_end_to_end_data_flow_with_real_structure() -> None:
    ...
    assert dsl_signal["reason"] == "Nuclear: ✓ SPY RSI(10)>79 → ✓ TQQQ RSI(10)<81 → 75.0% allocation"
    ...
    if strategy_signals_section_start != -1:
        ...
        assert "Nuclear:" in section or "SPY RSI" in section, "Decision path should be in Strategy Signals table"
...
def test_consolidated_portfolio_flattening_with_decimals() -> None:
    ...
    assert float(adapter.consolidated_portfolio["SPY"]) == 0.6
    assert float(adapter.consolidated_portfolio["BND"]) == 0.4
    # Decimals are okay - templates convert to float when needed
    assert isinstance(adapter.consolidated_portfolio["SPY"], (int, float, Decimal))
    assert isinstance(adapter.consolidated_portfolio["BND"], (int, float, Decimal))
