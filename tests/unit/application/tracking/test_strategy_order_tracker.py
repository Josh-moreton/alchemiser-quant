...
class TestStrategyOrderTracker:
    ...
    def test_partial_fills_aggregation(self, mock_get_s3_handler):
        ...
        assert position.quantity == 100.0  # 50 + 50
        assert position.average_cost == 202.5  # (50*200 + 50*205) / 100
    ...
    def test_realized_pnl_calculation(self, mock_get_s3_handler):
        ...
        assert abs(tracker._realized_pnl_cache["NUCLEAR"] - expected_pnl) < 0.01
    ...
    def test_strategy_pnl_summary(self, mock_get_s3_handler):
        ...
        assert pnl.realized_pnl == 250.0  # 25 * (160 - 150)
        assert pnl.positions["AAPL"] == 75.0  # 100 - 25 remaining
        
        # Check unrealized P&L: 75 shares * (155 - 150) = 375
        assert abs(pnl.unrealized_pnl - 375.0) < 0.01
    ...
    def test_decimal_usage_in_mappings(self, mock_get_s3_handler):
        ...
        assert abs(summary.total_qty - Decimal("33.333")) < Decimal("0.001")
        ...
        assert abs(event.quantity - Decimal("33.333")) < Decimal("0.001")
        assert abs(event.price - Decimal("523.45")) < Decimal("0.01")