...
class TestNuclearEngineErrorHandling:
    ...
    def test_generate_signals_with_market_data_failure(
        ...
        signals = nuclear_engine.generate_signals(mock_port, timestamp)
    ...
    def test_error_handler_integration(
        ...
        with patch.object(nuclear_engine, 'generate_signals', side_effect=Exception("Forced test error")):
            signals = nuclear_engine.safe_generate_signals(mock_port, datetime.now())
            assert signals == []
    ...
    def test_structured_error_contexts(
        ...
        with patch.object(nuclear_engine, '_calculate_indicators', side_effect=Exception("Test indicator error")):
            nuclear_engine.safe_generate_signals(mock_port, timestamp)
...
class TestKLMEngineErrorHandling:
    ...
    def test_generate_signals_with_market_data_failure(
        ...
        signals = klm_engine.generate_signals(mock_port, timestamp)
        assert isinstance(signals, list)
    ...
    def test_error_handler_integration(
        ...
        signals = klm_engine.safe_generate_signals(mock_port, timestamp)
        assert isinstance(signals, list)
