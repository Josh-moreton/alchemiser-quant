...
from the_alchemiser.domain.strategies.nuclear_signals import (
    NuclearStrategyEngine as LegacyNuclearStrategy,
)
...
class MockDataProviderToPortAdapter:
    ...
    def get_data(
        self, symbol: str, timeframe: str = "1day", period: str = "1y", **kwargs: Any
    ) -> pd.DataFrame:
...
class TestNuclearStrategyParity:
    ...
    def _create_normal_market_data(self, symbol: str) -> pd.DataFrame:
        """Create normal market data to avoid triggering other oversold/overbought conditions."""
        base_price = {
            "SPY": 450.0,
            "IOO": 80.0,
            "TQQQ": 50.0,
        ...
        # Create 300 days of very stable market data to keep RSI neutral (around 50)
        dates = pd.date_range("2023-01-01", periods=300, freq="D")
        data = []

        for i in range(300):
            # Very small price movements to keep RSI around 50
            price_mult = 1.0 + (0.001 * (i % 10 - 5))  # Small oscillations
            price = base_price * price_mult
            data.append(
                {
                    "Open": price * 0.9995,
    ...
    def test_parity_spy_oversold_scenario(
        ...
        def get_data_side_effect(symbol: str) -> pd.DataFrame:
            if symbol == "SPY":
                return spy_data
            # Create normal data for other symbols to avoid TQQQ oversold condition
            return self._create_normal_market_data(symbol)
    ...
    def test_parity_bull_market_scenario(
        ...
        def get_data_side_effect(symbol: str) -> pd.DataFrame:
            if symbol == "SPY":
                df = self._create_normal_market_data(symbol)
    ...
    def test_edge_case_missing_spy_data_parity(
        ...
        assert (
            "missing" in typed_signal.reasoning.lower() or "spy" in typed_signal.reasoning.lower()
        )
    ...
    def test_confidence_and_allocation_generation(
        ...
        if signal.action == "BUY":
            assert signal.confidence.value >= Decimal(
                "0.5"
            )  # Should have reasonable confidence for buy signals
