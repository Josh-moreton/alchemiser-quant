...
class TestCalculateDynamicLimitPrice:
    ...
    def test_buy_order_step_zero(self):
        ...
        result = calculate_dynamic_limit_price(side_is_buy=True, bid=100.0, ask=100.10, step=0)
        # Midpoint = 100.05
    ...
    def test_sell_order_step_zero(self):
        ...
        result = calculate_dynamic_limit_price(side_is_buy=False, bid=100.0, ask=100.10, step=0)
        # Midpoint = 100.05
        assert result == 100.05
    ...
    def test_buy_order_incremental_steps(self):
        ...
        price0 = calculate_dynamic_limit_price(side_is_buy=True, bid=100.0, ask=100.10, step=0)

        # Step 1: slightly higher
        price1 = calculate_dynamic_limit_price(side_is_buy=True, bid=100.0, ask=100.10, step=1)

        # Step 2: even higher
        price2 = calculate_dynamic_limit_price(side_is_buy=True, bid=100.0, ask=100.10, step=2)
    ...
    def test_sell_order_incremental_steps(self):
        ...
        # Step 0: near midpoint
        price0 = calculate_dynamic_limit_price(side_is_buy=False, bid=100.0, ask=100.10, step=0)

        # Step 1: slightly lower
        price1 = calculate_dynamic_limit_price(side_is_buy=False, bid=100.0, ask=100.10, step=1)

        # Step 2: even lower
        price2 = calculate_dynamic_limit_price(side_is_buy=False, bid=100.0, ask=100.10, step=2)
    ...
    def test_result_rounded_to_two_decimals(self):
        ...
        result = calculate_dynamic_limit_price(side_is_buy=True, bid=100.123, ask=100.456, step=0)
...
class TestCalculateAllocationDiscrepancy:
    ...
    def test_overweight_position(self):
        ...
        current_weight, diff = calculate_allocation_discrepancy(0.25, 3000.0, 10000.0)
    ...
    def test_underweight_position(self):
        ...
        current_weight, diff = calculate_allocation_discrepancy(0.25, 2000.0, 10000.0)
    ...
    def test_correctly_weighted_position(self):
        ...
        current_weight, diff = calculate_allocation_discrepancy(0.25, 2500.0, 10000.0)
    ...
    def test_zero_current_value(self):
        ...
        current_weight, diff = calculate_allocation_discrepancy(0.25, 0.0, 10000.0)
    ...
    def test_zero_portfolio_value_returns_target(self):
        ...
        current_weight, diff = calculate_allocation_discrepancy(0.25, 1000.0, 0.0)
        assert current_weight == 0.0
        assert diff == 0.25
...
class TestCalculateRebalanceAmountsDecimal:
    ...
    def test_basic_rebalancing_plan(self):
        ...
        target = {"AAPL": Decimal("0.5"), "MSFT": Decimal("0.3"), "CASH": Decimal("0.2")}
        current = {"AAPL": Decimal("4000"), "MSFT": Decimal("4000"), "CASH": Decimal("2000")}
    ...
    def test_needs_rebalance_threshold(self):
        ...
        result = calculate_rebalance_amounts_decimal(target, current, portfolio, threshold)
        ...
        result2 = calculate_rebalance_amounts_decimal(target, current2, portfolio, threshold)
    ...
    def test_multiple_symbols_rebalancing(self):
        ...
        assert len(result) == 4
        assert all(symbol in result for symbol in target)
...
class TestCalculatePositionSizeDecimal:
    ...
    def test_zero_price_returns_zero(self):
        ...
        result = calculate_position_size_decimal(Decimal("0.0"), Decimal("0.5"), Decimal("10000.0"))
    ...
    def test_precision_vs_float(self):
        ...
        result_float = calculate_position_size(float(price), float(weight), float(account))
...
class TestCalculateAllocationDiscrepancyDecimal:
    ...
    def test_decimal_precision(self):
        ...
        current_weight, diff = calculate_allocation_discrepancy_decimal(target, current, portfolio)
...
class TestTradingMathProperties:
    ...
    @pytest.mark.property
    @given(st.floats(min_value=90.0, max_value=110.0), st.floats(min_value=90.0, max_value=110.0))
    def test_dynamic_limit_price_within_spread(self, bid, ask):
        ...
        for step in range(6):
            price = calculate_dynamic_limit_price(side_is_buy=True, bid=bid, ask=ask, step=step)
            # Price should be between bid and ask (with some tolerance for rounding)
            assert bid - 0.01 <= price <= ask + 0.01

    @pytest.mark.property
    @given(st.floats(min_value=10.0, max_value=1000.0), st.floats(min_value=1.0, max_value=100.0))
    ...
    def test_allocation_discrepancy_bounded(self, target, current, portfolio):
        ...
        current_weight, diff = calculate_allocation_discrepancy(target, current, portfolio)
