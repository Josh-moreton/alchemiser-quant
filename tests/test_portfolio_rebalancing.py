...
class TestFullRebalance:
    ...
    def test_rebalance_from_cash_to_positions(
        self, order_manager, mock_trading_client, mock_data_provider
    ):
    ...
    def test_rebalance_between_positions(
        self, order_manager, mock_trading_client, mock_data_provider
    ):
        ...
        mock_trading_client.get_account.return_value = MagicMock(
            buying_power=1000.0, cash=0.0, portfolio_value=10000.0
        )
...
class TestPartialRebalance:
    ...
    def test_small_allocation_change(self, multi_strategy_trader, mock_trading_client):
        ...
        multi_strategy_trader.order_manager.alpaca_client.trading_client.submit_order.return_value = MagicMock(
            id="test_order_123"
        )
        ...
        mock_account = MagicMock(buying_power=1000.0, cash=0.0, portfolio_value=10000.0)
        ...
        assert (
            multi_strategy_trader.order_manager.alpaca_client.trading_client.submit_order.call_count
            == 0
        )
...
class TestEdgeCases:
    ...
    def test_new_symbol_addition(self, multi_strategy_trader, mock_trading_client):
        ...
        target_allocations = {"AAPL": 0.6, "MSFT": 0.4}  # New symbol
    ...
    def test_insufficient_cash_for_rebalance(self, multi_strategy_trader, mock_trading_client):
        ...
        mock_trading_client.get_account.return_value = MagicMock(
            buying_power=100.0,
            cash=50.0,
            portfolio_value=5050.0,  # Very low buying power
        )

        # Target: Add expensive stock
        target_allocations = {"AAPL": 0.5, "GOOGL": 0.5}  # Expensive stock
    ...
    def test_fractional_shares_rebalance(self, multi_strategy_trader, mock_trading_client):
        ...
        mock_trading_client.get_account.return_value = MagicMock(
            buying_power=1000.0, cash=0.0, portfolio_value=5000.0
        )

        target_allocations = {"AAPL": 0.7, "GOOGL": 0.3}
