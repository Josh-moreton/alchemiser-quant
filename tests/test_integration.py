...
def mock_trading_client():
    ...
    client.get_account.return_value = MagicMock(
        buying_power=10000.0, cash=10000.0, portfolio_value=10000.0
    )
...
class TestPaperTradingIntegration:
    ...
    def test_complete_paper_trading_session(
        self, mock_trading_client, mock_data_provider, mock_strategies
    ):
        ...
        with patch("the_alchemiser.execution.alchemiser_trader.TradingEngine") as MockTrader:
            ...
            mock_trader_instance.execute_strategies.return_value = {
                "trading_summary": {"total_orders": 2, "successful_orders": 2, "failed_orders": 0}
            }
    ...
    def test_paper_trading_with_market_data(self, mock_trading_client, mock_data_provider):
        ...
        mock_data_provider.get_current_price.side_effect = lambda symbol: price_data.get(
            symbol, 100.0
        )
...
class TestLiveTradingSimulation:
    ...
    def test_live_trading_safety_checks(self, mock_trading_client, mock_data_provider):
        ...
        mock_trading_client.get_account.return_value = MagicMock(
            buying_power=1000.0,
            cash=1000.0,
            portfolio_value=2000.0,  # Limited funds for safety
        )
...
class TestBacktestMode:
    ...
    def test_backtest_with_historical_data(self, mock_data_provider, mock_strategies):
        ...
        historical_data.index = [datetime.now() - timedelta(days=i) for i in range(30, 0, -1)]
        ...
        backtest_client.get_account.return_value = MagicMock(
            buying_power=10000.0, portfolio_value=10000.0
        )
        ...
        for day in range(30):
            ...
            order_id = order_manager.place_order("AAPL", 1.0, OrderSide.BUY)
            backtest_results.append({"day": day, "price": current_price, "order_id": order_id})
    ...
    def test_backtest_portfolio_evolution(self, mock_data_provider):
        ...
        portfolio_states = []
        for day in range(10):
            positions = [MagicMock(symbol="AAPL", qty=day * 10.0, market_value=day * 1500.0)]
    ...
    def test_backtest_performance_metrics(self, mock_data_provider):
        ...
        for day in range(20):
            ...
            account_states.append(
                MagicMock(portfolio_value=portfolio_value, buying_power=portfolio_value * 0.5)
            )
...
class TestEndToEndWorkflows:
    ...
    def test_morning_portfolio_rebalance(
        self, mock_trading_client, mock_data_provider, mock_strategies
    ):
    ...
    def test_intraday_strategy_execution(
        self, mock_trading_client, mock_data_provider, mock_strategies
    ):
        ...
        for hour, price in enumerate(price_timeline):
            ...
            execution_results.append(
                {"hour": hour, "price": price, "action": signal["action"], "order_id": order_id}
            )
    ...
    def test_end_of_day_settlement(self, mock_trading_client, mock_data_provider):
        ...
        pending_orders = [{"order_id": "order_1"}, {"order_id": "order_2"}, {"order_id": "order_3"}]
        ...
        settlement_result = order_manager.wait_for_settlement(
            pending_orders, max_wait_time=1, poll_interval=0.1
        )
