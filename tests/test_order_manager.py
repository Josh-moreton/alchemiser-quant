...
def order_manager():
    ...
    trading_client.get_order_by_id.side_effect = lambda order_id: MagicMock(
...
def test_calculate_dynamic_limit_price(order_manager):
    price = order_manager.calculate_dynamic_limit_price(
        OrderSide.BUY, 99.0, 101.0, step=1, tick_size=0.2, max_steps=3
    )
    assert price == 100.2
    price = order_manager.calculate_dynamic_limit_price(
        OrderSide.SELL, 99.0, 101.0, step=2, tick_size=0.5, max_steps=3
    )
    assert price == 99.0
